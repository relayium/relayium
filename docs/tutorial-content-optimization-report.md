...
> **⚠️ 这是一份有日期的历史审查，不是当前口径。** 文中关于配对码 TTL 的事实（"5 分钟"）
> 反映的是 2026-07-26 当天的实现快照。**当前 TTL 是 30 分钟**，唯一权威来源是
> `server/internal/signal` 的 `CodeTTLSeconds`。这里保留原文不改写，是因为审查记录的价值
> 在于"当时看到了什么"；要看现在的数字，读那个常量。
