...
> **⚠️ 这是一份有日期的历史审查，不是当前口径。** 下面的正文是 2026-07-26 当天的实现
> 快照，原样保留不改写——审查记录的价值在于"当时看到了什么"。
>
> **当前实现（截至 2026-08-02）**：配对码是**恰好 6 位纯十进制数字**，有效期 **5 分钟**。
> 唯一权威来源是 `server/internal/signal` 的 `CodeAlphabet`（`"0123456789"`)、`CodeLen`（`6`)、
> `CodeTTLSeconds`（`300`)——三个常量分别被 `pair_test.go` 和 `pair_ttl_test.go` 钉住。
>
> 据此读正文里的两类事实：字符集已经变了，正文写的字母数字集
> （`ACDEFHJKMNPRTWXY23456789`)不再是现在的口径；TTL 的"5 分钟"虽然与现在的数字相同，
> 但中间曾被放宽到 30 分钟又收回，所以它是巧合而不是依据。要看现在的数字，读那三个常量。

> **实施状态（2026-08-05 逐条对代码复核）：15 条里 11 条已完成，3 条部分完成，2 条未做。**
...
> **2026-08-05 实测更正（本节的篇数已经过时，且是往少了说）**：
> 提到跨网 TURN 路径的实际是 **19 篇**（本节写 7 篇），提到浏览器内存/落盘限制的实际是
> **10 篇**（本节写 6 篇），教程类文章总数是 **25 篇**（本报告统计时 24 篇，之后新增了
> `howto-send-text-between-devices`）。差额来自审查之后新增的文章——它们抄的是同一段。
> 复核命令：`grep -l "TURN relay" *.mjs | wc -l`、`grep -l "256 MiB\|memory" *.mjs | wc -l`，
> 在 `web/scripts/pages/content/articles/` 下跑。
>
> 这条差额本身就是本节论点的证据：**样板段落会随新文章继续繁殖**，所以去重必须做成引用
> （snippet），而不是一次性把现有 N 篇改一遍。
>
> 另：**配对码 5 分钟有效期**在 7 篇浏览器配对 how-to 里只有 2 篇写了
> （`howto-transfer-by-qr-code`、`howto-mac-to-windows`），其余 5 篇既没写有效期也没写码从哪来。
...
11. 修"无需代码"误译；建立术语表（配对码/校验码/守护进程直连/阅后即焚/会合）并在构建期或 review 中强制；翻译 zh 代码块注释。
   ...
   > **zh 半角标点守卫已补（2026-08-05 同日）**：最初以为它需要先能区分正文与代码块，
   > 这个判断是错的——规则按「CJK 邻接」判定，zh 串里合法的 Latin 片段、路径、flag、
   > 千分位数字天然不在射程内；连代码块一起量，四种形态命中都是 0。三条规则用报告
   > §1.3 里真实上线过的机翻残留做了注入证明，同时验证合法写法不误报。
   > **fr 不间断空格也已完成（2026-08-05 同日）**：原先记的「约 950 处」是错的——那是数
   > `fr.ts` **源码**里的冒号，而其中绝大多数是对象语法。按解析后的语料实测是 122 条字符串，
   > 已全部改为 U+00A0 并由同一个 lint 的 `; ! ?` 规则守住。冒号不纳入强制：唯三保留裸冒号的
   > 位置是引用的机器输出（`relayium-node: command not found`、`environment:` 配置键），
   > 九个语言都是这么写的，加空格反而是错的。
