...
- Progress reporting for cross-network transfers (reuse Phase-1 `SendOpts.Progress`; wire a TTY bar later). **Status 2026-09-21: completed for the sender** — `runSendCross` passes a `sendProgress` reporter (`server/cmd/relayium/progress.go`), tested through the `crossnetSendDial` seam. Receiver side completed the same day: `xfer.RecvOpts.Progress` is a purely observational hook, wired into `pull` and cross-network `receive` (`__recv` and `serve` stay silent by design).
