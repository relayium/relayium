...
- `push -` (stdin body) and `pull` of a single file to stdout: parsing exists; streaming a stdin source of unknown size needs a manifest tweak (`Size == -1` → stream-until-EOF frame). Add as a fast-follow task once the batch path is proven. **Status 2026-09-23: partial (R-C14 phase 1 of 3).** `pull [user@]host:file -` is implemented on branch `work/pull-stdout-20260923` (not yet on `main`, not in any release): exactly one remote regular file to stdout over the unchanged v1 wire, every other source refused with zero payload bytes written to stdout (an older remote may still start streaming before the refusal takes effect; pull aborts the connection), the ssh session aborted and its child reaped in bounded time on every failure, a terminal stdout refused. Still open: `push -` with an unknown-length stdin over SSH (phase 2) and the daemon-direct `relayium://` stdin form (phase 3); neither exists yet.
