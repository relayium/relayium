...
- 月度 relay 带宽"兵总额"硬切点: **数十 GB / 月**。到顶后**关闭该主机的 relay**(其同进程/同端口 3478 的 STUN 随之停);但**系统级 STUN 仍可用** —— 客户端还有默认公共 STUN 和池里其它 relay 主机的 STUN,同网段 / 能 NAT 打洞的传输照常工作,只是这台主机的付费转发停了。详见 Layer 0。
