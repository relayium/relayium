...
| **CLI / node release tags** | `.github/workflows/auto-release.yml` cuts a **weekly, green-gated tag**; `release.yml` builds and signs what that tag names | **No.** Merging cuts no tag. An urgent CLI/node fix reaches users only when a tag is cut for it. The tag is withheld unless two gates pass, and neither is bypassed by the `force` input: every check run on HEAD is green (`scripts/release/checks-green.sh`), and a successful `go.yml` run exists on a commit whose Go-lane inputs are identical to HEAD's (`scripts/release/go-evidence.sh`) — HEAD may be a docs commit on which `go.yml` never ran. A refusal is written to the run's step summary with its reason; when Go evidence is what is missing, the remedy is `gh workflow run go.yml --ref main`, wait for success, then run `auto-release` again. |
