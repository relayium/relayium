...
`/admin` 后台本身只有在设置了 `RELAYIUM_ADMIN_PASS`(管理员密码)时才会挂载路由;密码为空时 `/admin` 路由根本不注册,请求会落到 SPA 的兜底处理,返回普通的前端首页外壳(HTTP 200),而不是任何管理页面。
...
一旦某个验证码被成功用于登录,该时间步以及更早的所有时间步的验证码都会立即失效(服务端记录已接受过的最近时间步并单调前进),无法再被重放;所以截获到的验证码即使还在有效期内也无法重复使用。
