...
(`CreateStoredFileWithinStorageCaps`, `account/sqlite.go:5267`). Only the traffic one is shared with
...
| `usage_periods` (`sqlite.go:1695`) | the same relay data bucketed by calendar month (`YYYYMM`), which is what billing/cap queries actually read (`account/plan_enforce.go:64`, `UserRelayedSince`). |
...
- **Daily upload quota** — a rolling 24-hour window (`account/plan_enforce.go:344`,
  ...
  (`CreateStoredFileWithinStorageCaps`, `account/sqlite.go:5267`), so
...
| Account + all of the above, on deletion | A grace period after a self-deletion request (`-account-grace-days` / `RELAYIUM_ACCOUNT_GRACE_DAYS`, default 30 days, `main.go:338`), then hard-purged | `ArchiveAndPurgeUser`, `account/sqlite.go:3045` |
