...
starts (`main.go:922`) and relay usage is never ingested at all — transfers
...
| `usage_events` (`sqlite.go:105`) | per-TURN-allocation relayed-byte totals: alloc ID, token, user ID, bytes, timestamp, later `node_id` and `billable` (`sqlite.go:402`). |
| `usage_periods` (`sqlite.go:1675`) | the same relay data bucketed by calendar month (`YYYYMM`), which is what billing/cap queries actually read (`account/plan_enforce.go:63`, `UserRelayedSince`). |
| `usage_monthly` (`sqlite.go:147`) | per-user, per-month upload/download byte totals for **stored transfers** (not relay). |
...
| `nodes` (`sqlite.go:188`) | self-hosted or fleet relay/storage node registry: owner, region, URLs, per-node relayed/stored byte totals, online status. |
| `cli_device_auth` (`sqlite.go:251`) | the CLI's device-code login flow: **the requesting CLI's origin IP and user-agent**, shown on the browser approval page so a user can spot a phishing attempt (`sqlite.go:261-262`). This is the one place a general client IP is persisted — see [What the server could know but chooses not to keep](#what-the-server-could-know-but-chooses-not-to-keep). |
| `admin_audit` (`sqlite.go:307`) | every admin-console mutation: actor, **the admin's IP**, action, target, and a diff of what changed. Kept for up to 2 years by default — see [Retention](#retention-how-long-anything-is-kept). |
| `plans` (`sqlite.go:285`) | the tier table itself: storage/traffic/retention caps, prices, Stripe price IDs. No user data. |
...
- **Byte counts are the metered quantity, not access logs.** There's no table
  ...
  `download_receipts` (`sqlite.go:500`) exists, but it's a 24-hour dedup table
...
| Admin audit trail (`admin_audit`) | 2 years by default, admin-overridable (`-audit-retention-days` / `RELAYIUM_AUDIT_RETENTION_DAYS`, `main.go:257`) | `auditRetentionDefault`, `account/gc.go:64`, applied at `account/gc.go:169` |
...
| Account + all of the above, on deletion | A grace period after a self-deletion request (`-account-grace-days` / `RELAYIUM_ACCOUNT_GRACE_DAYS`, default 30 days, `main.go:245`), then hard-purged | `ArchiveAndPurgeUser`, `account/sqlite.go:2861` |

**What "hard-purged" actually does**, read directly from
`ArchiveAndPurgeUser` (`account/sqlite.go:2861-3065`): the user's monthly
stored-transfer totals are folded into `usage_archive` — **period totals
only, with no user ID retained** (`sqlite.go:2886-2890`) — and then every
...
itself. The comment at `sqlite.go:2905-2911` is explicit that leaving
...
- **Relay metering** is entirely off unless `-redis-addr` /
  `RELAYIUM_REDIS_ADDR` is set (`main.go:193`, wired at `main.go:922`). No
...
- **TURN relay itself** is off unless `-turn-secret` /
  `RELAYIUM_TURN_SECRET` is set (`main.go:188`) — without it there's simply no
...
- **Billing (Stripe)** is entirely off unless `-stripe-secret-key` /
  `RELAYIUM_STRIPE_SECRET_KEY` is set (`main.go:262`); every
