...
| `usage_periods` (`sqlite.go:1417`) | the same relay data bucketed by calendar month (`YYYYMM`), which is what billing/cap queries actually read (`account/plan_enforce.go:63`, `UserRelayedSince`). |
...
| Admin audit trail (`admin_audit`) | 2 years by default, admin-overridable (`-audit-retention-days` / `RELAYIUM_AUDIT_RETENTION_DAYS`, `main.go:159`) | `auditRetentionDefault`, `account/gc.go:64`, applied at `account/gc.go:169` |
...
| Account + all of the above, on deletion | A grace period after a self-deletion request (`-account-grace-days` / `RELAYIUM_ACCOUNT_GRACE_DAYS`, default 30 days, `main.go:147`), then hard-purged | `ArchiveAndPurgeUser`, `account/sqlite.go:2549` |

**What "hard-purged" actually does**, read directly from
`ArchiveAndPurgeUser` (`account/sqlite.go:2549-2678`): the user's monthly
stored-transfer totals are folded into `usage_archive` — **period totals
only, with no user ID retained** (`sqlite.go:2573-2578`) — and then every
...
itself. The comment at `sqlite.go:2593-2599` is explicit that leaving
...
- **Billing (Stripe)** is entirely off unless `-stripe-secret-key` /
  `RELAYIUM_STRIPE_SECRET_KEY` is set (`main.go:164`); every
