...
| `usage_periods` (`sqlite.go:1015`) | the same relay data bucketed by calendar month (`YYYYMM`), which is what billing/cap queries actually read (`account/plan_enforce.go:63`, `UserRelayedSince`). |
...
| Account + all of the above, on deletion | A grace period after a self-deletion request (`-account-grace-days` / `RELAYIUM_ACCOUNT_GRACE_DAYS`, default 30 days, `main.go:140`), then hard-purged | `ArchiveAndPurgeUser`, `account/sqlite.go:2023` |

**What "hard-purged" actually does**, read directly from
`ArchiveAndPurgeUser` (`account/sqlite.go:2023-2101`): the user's monthly
stored-transfer totals are folded into `usage_archive` — **period totals
only, with no user ID retained** (`sqlite.go:1952-1959`) — and then every
...
itself. The comment at `sqlite.go:1972-1978` is explicit that leaving
