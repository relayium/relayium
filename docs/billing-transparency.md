...
(`CreateStoredFileWithinStorageCaps`, `account/sqlite.go:5375`). Only the traffic one is shared with
...
| `usage_periods` (`sqlite.go:1780`) | the same relay data bucketed by calendar month (`YYYYMM`), which is what billing/cap queries actually read (`account/plan_enforce.go:64`, `UserRelayedSince`). |
...
- **Daily upload quota** — a rolling 24-hour window (`account/plan_enforce.go:344`,
  ...
  (`CreateStoredFileWithinStorageCaps`, `account/sqlite.go:5375`), so
...
- **Uploads in flight when an account is deleted** — every single-shot
  ...
  the transaction that would store it (`UploadFence`, `account/store.go:790`):
...
| Account + all of the above, on deletion | A grace period after a self-deletion request (`-account-grace-days` / `RELAYIUM_ACCOUNT_GRACE_DAYS`, default 30 days, `main.go:338`), then hard-purged | `ArchiveAndPurgeUser`, `account/sqlite.go:3077` |

**What "hard-purged" actually does**, read directly from
`ArchiveAndPurgeUser` (`account/sqlite.go:3026-3231`): the user's monthly
