...
(`CreateStoredFileWithinStorageCaps`, `account/sqlite.go:5207`). Only the traffic one is shared with
...
- **Daily upload quota** — a rolling 24-hour window (`account/plan_enforce.go:344`,
  ...
  (`CreateStoredFileWithinStorageCaps`, `account/sqlite.go:5207`), so
  ...
  metered upload and download traffic and to stored bytes. Reading it needs
  no blob access: it is served as an empty body without reading storage, and
  a chunked upload that sends no bytes need not create a blob at all (a
  single-shot empty upload still writes a zero-byte file).
...
| Account + all of the above, on deletion | A grace period after a self-deletion request (`-account-grace-days` / `RELAYIUM_ACCOUNT_GRACE_DAYS`, default 30 days, `main.go:338`), then hard-purged | `ArchiveAndPurgeUser`, `account/sqlite.go:3031` |
