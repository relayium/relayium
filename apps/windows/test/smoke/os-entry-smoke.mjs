...
const driver = fileURLToPath(new URL("./os-entry-main.mjs", import.meta.url));
...
const owned = [
  mkdtempSync(path.join(tmpdir(), "relayium-os-entry-")),
  mkdtempSync(path.join(tmpdir(), "relayium-os-entry-profile-")),
];
...
function finish(code, message) {
  ...
  if (stuck.length > 0) {
    process.stderr.write(
      `os-entry: could not remove ${stuck.length} owned directory(ies)\n${stuck.join("\n")}\n`,
    );
    process.exit(1);
  }
  process.exit(code);
...
async function buildHarness() {
  await build({
    ...
    build: {
      ...
      lib: {
        entry: path.join(appRoot, "src/renderer/os-entry/harness-entry.ts"),
        formats: ["iife"],
        name: "RelayiumOsEntryHarness",
        fileName: () => "harness.js",
      },
    },
  });
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Relayium OS-entry harness</title>
${links}
  </head>
  <body>
    <div id="app"></div>
...
const built = await buildHarness().catch((err) => {
  finish(1, `os-entry: the harness did not build\n${String(err?.stack ?? err)}\n`);
  return null;
});
if (built === null) process.exit(1);
if (built.css.length === 0) {
  finish(1, "os-entry: the build produced no stylesheet, so nothing would be styled\n");
}
if (buildOnly) {
  process.stdout.write(`os-entry: harness built (${built.css.join(", ")})\n`);
  finish(0, null);
}
...
child.on("exit", (code) => {
  clearTimeout(timer);
  if (timedOut) {
    finish(1, `os-entry: no result within ${TIMEOUT_MS}ms\n${out}\n${err}\n`);
    return;
  }
  const line = out.split("\n").find((l) => l.startsWith("RELAYIUM_OS_ENTRY "));
  if (!line) {
    finish(1, `os-entry: produced no result line (exit ${code})\n${out}\n${err}\n`);
    return;
  }
  const { failures, checks } = JSON.parse(line.slice("RELAYIUM_OS_ENTRY ".length));
  if (failures.length > 0) {
    finish(1, `os-entry: ${failures.length} failed\n${failures.join("\n")}\n`);
    return;
  }
  if (code !== 0) {
    finish(1, `os-entry: assertions passed but the renderer exited ${code}\n${err}\n`);
    return;
  }
  process.stdout.write(`os-entry: ${checks} renderer assertions passed\n`);
  finish(0, null);
});
