// Build the LINK PANE with the real Svelte CLIENT build, then drive it in a
// real Electron renderer.
//
// The realtime lane does render this screen — against a real server, a real
// browser peer and a real transport — but it can only reach the states a
// healthy pairing actually produces. It cannot expire a relay credential, drop
// a signalling socket, make a peer flood the text lane, or hand back each of
// the ten publish-failure reasons in turn. Every one of those has copy, and
// until this step none of it had ever been displayed.
//
...
const driver = fileURLToPath(new URL("./link-pane-main.mjs", import.meta.url));
...
const owned = [
  mkdtempSync(path.join(tmpdir(), "relayium-link-pane-")),
  mkdtempSync(path.join(tmpdir(), "relayium-link-pane-profile-")),
];
...
function finish(code, message) {
  ...
  if (stuck.length > 0) {
    process.stderr.write(
      `link-ui: could not remove ${stuck.length} owned directory(ies)\n${stuck.join("\n")}\n`,
    );
    process.exit(1);
  }
  process.exit(code);
...
async function buildHarness() {
  await build({
    ...
    build: {
      ...
      lib: {
        entry: path.join(appRoot, "src/renderer/link/harness-entry.svelte.ts"),
        formats: ["iife"],
        name: "RelayiumLinkHarness",
        fileName: () => "harness.js",
      },
    },
  });
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Relayium link-pane harness</title>
${links}
  </head>
  <body>
    <div id="app"></div>
...
const built = await buildHarness().catch((err) => {
  finish(1, `link-ui: the harness did not build\n${String(err?.stack ?? err)}\n`);
  return null;
});
if (built === null) process.exit(1);
if (built.css.length === 0) {
  finish(1, "link-ui: the build produced no stylesheet, so nothing would be styled\n");
}
if (buildOnly) {
  process.stdout.write(`link-ui: harness built (${built.css.join(", ")})\n`);
  finish(0, null);
}
...
child.on("exit", (code) => {
  clearTimeout(timer);
  if (timedOut) {
    finish(1, `link-ui: no result within ${TIMEOUT_MS}ms\n${out}\n${err}\n`);
    return;
  }
  const line = out.split("\n").find((l) => l.startsWith("RELAYIUM_LINK_UI "));
  if (!line) {
    finish(1, `link-ui: produced no result line (exit ${code})\n${out}\n${err}\n`);
    return;
  }
  const { failures, checks } = JSON.parse(line.slice("RELAYIUM_LINK_UI ".length));
  if (failures.length > 0) {
    finish(1, `link-ui: ${failures.length} failed\n${failures.join("\n")}\n`);
    return;
  }
  if (code !== 0) {
    finish(1, `link-ui: assertions passed but the renderer exited ${code}\n${err}\n`);
    return;
  }
  process.stdout.write(`link-ui: ${checks} renderer assertions passed\n`);
  finish(0, null);
});
