// Build the SAME-NETWORK page with the real Svelte CLIENT build, then drive it
// in a real Electron renderer.
//
// This screen had no renderer coverage of any kind. The realtime lane pairs by
// CODE — its driver clicks `nav-pair` — and the bootstrap smoke only asserts
// that `lan-start` is ABSENT when signed out. So every state here was
// compile-checked and never displayed: the empty roster, the offline card, the
// reconnecting card, the user-stopped card, the statement that stands in for a
// disabled Connect button, and the two sentences saying who else can be at the
// same public address.
//
...
const driver = fileURLToPath(new URL("./lan-page-main.mjs", import.meta.url));
...
const owned = [
  mkdtempSync(path.join(tmpdir(), "relayium-lan-page-")),
  mkdtempSync(path.join(tmpdir(), "relayium-lan-page-profile-")),
];
...
function finish(code, message) {
  ...
  if (stuck.length > 0) {
    process.stderr.write(
      `lan-ui: could not remove ${stuck.length} owned directory(ies)\n${stuck.join("\n")}\n`,
    );
    process.exit(1);
  }
  process.exit(code);
...
async function buildHarness() {
  await build({
    ...
    build: {
      ...
      lib: {
        entry: path.join(appRoot, "src/renderer/lan/harness-entry.svelte.ts"),
        formats: ["iife"],
        name: "RelayiumLanHarness",
        fileName: () => "harness.js",
      },
    },
  });
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <title>Relayium same-network harness</title>
${links}
  </head>
  <body>
    <div id="app"></div>
...
const built = await buildHarness().catch((err) => {
  finish(1, `lan-ui: the harness did not build\n${String(err?.stack ?? err)}\n`);
  return null;
});
if (built === null) process.exit(1);
if (built.css.length === 0) {
  finish(1, "lan-ui: the build produced no stylesheet, so nothing would be styled\n");
}
if (buildOnly) {
  process.stdout.write(`lan-ui: harness built (${built.css.join(", ")})\n`);
  finish(0, null);
}
...
child.on("exit", (code) => {
  clearTimeout(timer);
  if (timedOut) {
    finish(1, `lan-ui: no result within ${TIMEOUT_MS}ms\n${out}\n${err}\n`);
    return;
  }
  const line = out.split("\n").find((l) => l.startsWith("RELAYIUM_LAN_UI "));
  if (!line) {
    finish(1, `lan-ui: produced no result line (exit ${code})\n${out}\n${err}\n`);
    return;
  }
  const { failures, checks } = JSON.parse(line.slice("RELAYIUM_LAN_UI ".length));
  if (failures.length > 0) {
    finish(1, `lan-ui: ${failures.length} failed\n${failures.join("\n")}\n`);
    return;
  }
  if (code !== 0) {
    finish(1, `lan-ui: assertions passed but the renderer exited ${code}\n${err}\n`);
    return;
  }
  process.stdout.write(`lan-ui: ${checks} renderer assertions passed\n`);
  finish(0, null);
});
