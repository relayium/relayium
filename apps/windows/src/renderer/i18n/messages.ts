...
export const en = {
  ...
  pairJoin: "Join",
  pairCliPeer: "That code is paired with the relayium command-line tool, which connects only to another CLI. To move a file between the CLI and this app, run relayium up <file> and open the link it prints, or share a link from here and fetch it with relayium down.",
...
export const zh: Record<MessageKey, string> = {
  ...
  pairEnterCode: "输入配对码",
  pairJoin: "加入",
  pairCliPeer: "这个配对码来自 relayium 命令行工具，它只能与另一个命令行工具连接。要在命令行和本应用之间传文件：在命令行运行 relayium up <文件>，然后打开它打印的链接；或者从这里分享一个链接，再用 relayium down 取回。",
