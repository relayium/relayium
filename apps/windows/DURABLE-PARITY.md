...
| Tray | Opens the app and its three live surfaces, and pauses or resumes BOTH Nearby and the Device Inbox without opening the window. Pausing the Inbox stops claiming without writing the stored policy or telling central, so a mis-click in a menu costs a pause and not an enrolment; the label says what it will do and is rebuilt when the state moves. Driven against the real runtime in the resident smoke | It carries no STATUS: the macOS menu bar names the account and reports what each transfer surface is doing, and this one is actions only, so the window is still the only way to learn whether receiving is on. The reveal for the latest receipt is also menu-bar only |
