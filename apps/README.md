...
- `mac/` — macOS SwiftUI app (`com.relayium.mac`), depends on the local RelayiumKit
  package. Current public release **1.2.5** (GitHub Release `macos-v1.2.5`).
- `mac/RelayiumShare/` — the macOS Share Extension (`com.relayium.mac.Share`),
  ...
  inside the 1.2.5 app. The system Share menu is verified to list it; a
...
**Status: current release 1.2.5.** The owner requested this 1.2.5 release, the
recorded decision in `apps/mac/release-readiness.json` remains approved
(`"approved": true`), and the GitHub release workflow publishes the notarized
...
TestFlight, and 1.2.5 (12) is planned as the next candidate to succeed it, again
for internal TestFlight only; that build has not been archived or uploaded yet.
Nothing has been submitted for App Review, and
