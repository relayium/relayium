...
- `android/` — native Android client (`com.relayium.android`), Kotlin + Jetpack
  Compose over a pure-JVM `:protocol` module. **Public preview at 0.2.2
  (versionCode 5)**; direct APK only, no Google Play, no Play Services, no Play
...
That file has a second author — `web/src/lib/text-vectors.test.ts` prints its
`textKeys` block for a human to paste — so it is deliberately outside the gate
described next.

The cross-language wire vectors (`Tests/Fixtures/realtime-wire-vectors.json` from
`web/scripts/gen-realtime-wire-vectors.mjs`, `Tests/Fixtures/store-wire-vectors.json`
from `web/scripts/gen-store-wire-vectors.mjs`) are transcriptions of the Web
implementation, and they are gated. From `web/`:

- `npm run test:vectors` re-runs both generators twice and fails if the tracked
  bytes moved. It restores what it found, so it never edits your tree.
...
provisioning profile. That profile now exists — it produced the signed 1.2.4 (11)
package described above — but it is not required for an ordinary local check,
which builds unsigned:
