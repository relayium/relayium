...
final class MacSurfaceGuardTests: XCTestCase {
    ...
    func testNothingStillCallsTheMacLinkPathInline() throws {
        ...
        // The third stale copy of that sentence was in `apps/README.md`. A
        // document is not something this lane runs for, so that ban lives in
        // `scripts/test/document-claims-test.mjs`, which runs on every push.
    }
    ...
    //
    // The guards over `README.md`, `apps/README.md` and
    // `apps/mac/release-readiness.json` — no public iOS release claimed, the
    // published macOS tag and the Mac App Store version named from their own
    // records, the iOS status sentences, the old window floor, the merged
    // Workspace, overstated distribution, predicted figures — lived here as
    // nine XCTest cases plus one assertion of
    // `testNothingStillCallsTheMacLinkPathInline`. This class runs only in
    // `swift-package.yml`, which starts on `apps/RelayiumKit/**` alone, and
    // none of those documents is under it: on 2026-09-20 a docs-only commit
    // broke one of them, no lane that could notice started, and `main` was red
    // for a day. They now run on Linux in the unfiltered `repo-hygiene.yml`, on
    // every push. Do NOT add a document claim back here — it would be a second
    // copy that only runs when Swift changes.
