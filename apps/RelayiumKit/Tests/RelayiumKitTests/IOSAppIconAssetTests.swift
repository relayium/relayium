...
final class IOSAppIconAssetTests: XCTestCase {
    ...
    private var iconSet: URL {
        repoRoot.appendingPathComponent("apps/ios/Relayium/Assets.xcassets/AppIcon.appiconset")
    }
    ...
    func testThePNGIs1024SquareWithNoAlphaChannel() throws {
        ...
        let data = try Data(contentsOf: iconSet.appendingPathComponent(entry.filename!))
    ...
    func testEveryCornerIsPaintedRatherThanMasked() throws {
        let entry = try XCTUnwrap(try manifest().images.first)
        let url = iconSet.appendingPathComponent(entry.filename!) as CFURL
    ...
    func testTheCatalogIsDeclaredAndSharesOneRenderer() throws {
        let project = try String(
            contentsOf: repoRoot.appendingPathComponent("apps/ios/Relayium.xcodeproj/project.pbxproj"),
            encoding: .utf8)
        XCTAssertEqual(
        ...
        let renderer = try String(
            contentsOf: repoRoot.appendingPathComponent("apps/mac/tools/render-app-icon.swift"),
            encoding: .utf8)
