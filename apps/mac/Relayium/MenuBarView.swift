...
struct MenuBarView: View {
    ...
    /// Routing only, and for one item: the Device Inbox action below opens the
    /// main window ON its destination, which means selecting before the window is
    /// asked for. Nothing here renders a destination or reads which one is
    /// current.
    ...
    var body: some View {
        ...
        // **The route into the product, not into its settings.**
        //
        // This item used to open ⌘,, because the settings tab was the only full
        // Device Inbox surface there was — and on macOS 26.6 the selector it
        // reached for answered nowhere in this app's responder chain, so with
        // every window closed it produced no window at all while the menu closed
        // looking like it had acted. Both problems are gone with the destination:
        // `openWindow(id:)` addresses the unique `Window` scene directly and
        // needs no responder chain, no undocumented selector and no availability
        // split, and where it lands is a first-class destination rather than a
        // preferences pane the user has to know about.
        //
        // Selected BEFORE the window is asked for, so it opens already showing
        // the Device Inbox instead of rendering wherever the user last was and
        // moving under them.
...
///
/// A type rather than a method on `MenuBarView` because a `Scene`'s menu content
/// is rebuilt freely and this holds no state; it also keeps the one deprecated
/// call this app makes in a single declaration that can be marked as such.
