...
struct DeviceInboxSurface: View {
    ...
    var body: some View {
        // A stack of reference cards inside the destination scaffold's own
        // scroll view — the same vocabulary as every other destination, and one
        // scroller for the whole page.
        VStack(alignment: .leading, spacing: Metrics.section) {
            ...
            case .surface:
                ...
                if let peer = openPeer {
                    ...
                    // once. This is a resizable window with a scrolling page
            ...
            // OUTSIDE the switch and after it, so help is the last group in
            // every one of the three branches rather than one the signed-out
            // reader — the one most likely to need it — never sees.
    ...
    private var conversationsSection: some View {
        SectionCard(title: L10n.t(.inboxConversationsHeading),
                    titleIdentifier: "inbox-conversations",
                    rows: true) {
            CardRows {
                ...
                if inbox.conversations.isEmpty && !inbox.conversationStoreIssue {
                    // A card is never an empty box: the grouped list this
                    // replaced could show a bare heading, a card cannot.
                    CardBlockRow { SupportingText(L10n.t(.inboxConversationsEmpty)) }
                }
                CardRowList(inbox.conversations) { conversation in
    ...
    private func conversationRow(_ conversation: InboxConversation) -> some View {
        HStack(alignment: .center, spacing: 11) {
            GlyphChip(symbol: "laptopcomputer.and.iphone")
            VStack(alignment: .leading, spacing: 1) {
                HStack(spacing: 6) {
                    if conversation.unreadCount > 0 {
                        // The unread count below says it in words; the dot is
                        // the second carrier, in the app's one live colour.
                ...
                if conversation.unreadCount > 0 {
                    Text(L10n.t(.inboxConversationUnread,
                        ...
                        .font(.subheadline.weight(.medium)).foregroundStyle(Palette.actionLabel)
                }
            }
            ...
            // The platform's bordered button. On macOS 15 a press on this row's
            // reference-styled Open reached the scroll view's document view and
            // never the button, so the row's one navigation uses AppKit's own
            // control, which takes the press there.
    ...
    private func accountSection(_ gate: AccountGate) -> some View {
        // **Account, not Device Inbox.** The toolbar and the sidebar row both
        // already say Device Inbox, and this group is not the feature — it is
        // the one thing standing between the reader and it. Naming the
        // requirement is what the group's caption is for.
        //
        // The identifier is on the caption LEAF, never on the card. This branch
        // contains two buttons, and a container identifier renames every
        // control inside it — the propagation defect this pane has already lost
        // a control to twice.
        //
        // The footnote is what the feature actually does, said to somebody who
        // has not committed to it yet.
        SectionCard(title: L10n.t(.navAccount),
                    titleIdentifier: "inbox-signed-out",
                    footnote: L10n.t(.inboxExplain)) {
    ...
    ///
    /// Drawn as the reference's status head — the radar, the state in words and
    /// what the feature does — because it is this destination's one summary.
    private func statusSection(offersControls: Bool) -> some View {
        VStack(alignment: .leading, spacing: Metrics.inner) {
            HStack(alignment: .center, spacing: Metrics.rowHorizontal) {
                BrandRadar(symbol: symbol, isActive: isLive)
                VStack(alignment: .leading, spacing: 2) {
                    // The status line is the one sentence the acceptance suite
                    // reads the live state from, so it carries the identifier.
                    Text(InboxStatusPresentation.text(for: inbox.state))
                        .font(.title2.weight(.semibold))
                        .foregroundStyle(Palette.text)
                        .fixedSize(horizontal: false, vertical: true)
                        .accessibilityAddTraits(.isHeader)
                        .accessibilityIdentifier("inbox-status")
                    Text(L10n.t(.inboxExplain))
                        .font(.callout)
        ...
        // Named for what it answers — can this Mac take a delivery right now —
        // not for the feature, which the toolbar and sidebar already name.
        .accessibilityElement(children: .contain)
        .accessibilityLabel(L10n.t(.inboxStatusHeading))
    }

    /// Whether the radar is drawn live: the receiver is up and taking, or
    /// holding, deliveries. The words beside it carry the exact state.
    private var isLive: Bool {
        switch inbox.state {
        case .ready, .working, .asking, .saved, .savedMessage:
            return true
    ...
    /// The glyph in the status head's radar. The words beside it carry the
    /// state; the glyph — a warning triangle, a crossed octagon, no Wi-Fi — is
    /// the second carrier, so no state is told apart by colour alone.
    ...
    private var notificationSection: some View {
        if let notice = InboxNotificationPermissionPresentation
            ...
            SectionCard(title: L10n.t(.inboxNotificationsHeading)) {
                ...
                // it matches the recovery button the status head renders for
                // every other actionable state in this pane.
    ...
    private var askSection: some View {
        if !inbox.asking.isEmpty {
            SectionCard(title: L10n.t(.inboxAskHeading),
                        titleIdentifier: "inbox-ask",
                        footnote: L10n.t(.inboxAskExplain),
                        rows: true) {
                CardRows {
                    ...
                    // Keyed by each held delivery's own id, so answering one cannot
                    // hand its row to the next.
                    CardRowList(inbox.asking) { item in
                        let index = inbox.asking.map(\.id).firstIndex(of: item.id) ?? 0
                        HStack(spacing: 10) {
                            VStack(alignment: .leading, spacing: 2) {
    ...
    private var folderSection: some View {
        SectionCard(title: L10n.t(.inboxFolderHeading),
                    footnote: L10n.t(.inboxFolderExplain)) {
            HStack(spacing: 10) {
                GlyphChip(symbol: "folder")
    ...
    private var policySection: some View {
        // The group's own marker is on its caption, matching the Ask group: a
        // caption `Text` is a leaf, so it names the group without reaching into
        // the controls inside it.
        SectionCard(title: L10n.t(.inboxPolicyHeading),
                    titleIdentifier: "inbox-policy",
                    footnote: L10n.t(.inboxPolicyExplain)) {
            ...
            // A radio group: one visible row per choice, which is what the
            // grouped list's inline style drew, and the platform's own control
            // for one answer out of three.
            .pickerStyle(.radioGroup)
            // The label is kept and hidden, not removed. The enclosing group's
            // caption says *Receiving* two lines above, so an inline picker
    ...
    private var resultsSection: some View {
        SectionCard(title: L10n.t(.inboxResultsHeading)) {
    ...
    private var messagesSection: some View {
        if !inbox.messages.isEmpty {
            SectionCard(title: InboxMessagePresentation.heading(),
                        titleIdentifier: "inbox-messages",
                        footnote: InboxMessagePresentation.explanation()) {
                ...
                ForEach(Array(InboxMessagePresentation.shown(inbox.messages).enumerated()),
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        // The message, whole. `fixedSize` because a text row
