...
import { buildLegalPages, buildLandingPages, buildSitemap } from "./pages/build-pages.mjs";
...
async function main() {
  const pages = [...buildLegalPages(legalDocs), ...buildLandingPages(landing)];
  for (const page of pages) {
    const abs = join(publicDir, page.path);
    await mkdir(dirname(abs), { recursive: true });
    await writeFile(abs, page.html, "utf8");
  }
  await writeFile(join(publicDir, "sitemap.xml"), buildSitemap(legalDocs, { home: true, landing }), "utf8");
