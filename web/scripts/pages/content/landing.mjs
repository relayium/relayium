...
const zh = {
  ...
  faq: {
    ...
    items: [
      {
        q: "免费吗？",
        a: "软件本身免费开源（AGPL-3.0 许可），无需安装，你也可以自行部署。relayium.com 托管服务提供的是免费档位，而不是无限量的免费托管。同一网络下的实时文件与文本传输免费且无需登录，持配对码加入也始终不需要账号。创建跨网络配对码或存储下载链接需要登录，并会占用你所在档位的额度：每月流量额度（托管上传、托管下载与计费中继字节合并计算）、同时存放的加密数据上限、留存时长上限、每日上传上限——这四项各自独立。免费档位每项都有额度，Plus、Pro、Max 付费档位可以提高。",
      },
      {
        q: "文件会上传到服务器吗？",
