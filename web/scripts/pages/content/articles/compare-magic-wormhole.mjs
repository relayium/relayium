...
const ja = {
  ...
  sections: [
    {
      heading: "両者の共通点",
      body: [
        "どちらのツールも同じ核心的な課題を、同じ誠実な方法で解決しています。あなたが管理できないサーバーにファイルが留まることはなく、両端が帯域外で受け渡すのは短いコードだけです。magic-wormhole はアカウントが一切不要で、Relayium はサーバーがそのコードを発行できるよう送信側にのみサインインを求め、受信側は依然として不要です。",
      ],
      bullets: [
    ...
    },
    {
      ...
      body: [
        ...
        "Relayium の send/receive は直接接続専用です。ハンドシェイクの直後に数秒間だけ直接接続を試み、見つからなければリレーにフォールバックせずそのまま失敗します。設計上、Relayium のサーバーはネットワークをまたぐ CLI 転送のファイルバイトに一切触れません。実際にはこれは稀なケースです（ほとんどの家庭やオフィスのネットワークは直接経路を許可します）が、両方とも異常に厳格な NAT の内側にある2台のマシン間でファイルを移動する場合、magic-wormhole の方がそのまま動く可能性が高いです。直接経路が見つからず、リレーを避けることより確実性の方が重要なら、magic-wormhole を選ぶべき場面です。あるいは、直接 P2P ホップに依存しない Relayium の push/pull や、到達可能なサーバーへのデーモン直結を使うのも手です。",
      ],
    },
    {
      heading: "SSH とデーモン直結：すでに運用しているサーバーと話す",
      body: [
        "Relayium CLI が本当に付加価値を持つのは、一回限りのペアリングコードのケースの外側です。すでに持っているインフラを活かし、magic-wormhole がカバーしようとしていない2つの方法があります。",
    ...
    },
    {
      ...
      bullets: [
        "直接経路がない場合：magic-wormhole の Transit Relay は暗号化ストリームを運ぶので転送は完了します。一方、Relayium の send/receive は直接接続専用で、この場合は失敗します。",
        "サーバーとの対話：Relayium は SSH アクセス（push/pull）または証明書ピンニング付きの TLS のデーモンを再利用します。magic-wormhole には SSH との統合がなく、両端にインストールしてコードを共有する必要があります。",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "アカウントは必要ですか？",
        a: "send と、クラウドの up で必要です。push/pull は自分の SSH アクセスを使い、デーモン直結はマシン間の TLS 証明書ピンニングによる信頼を使うので、どちらも Relayium アカウントには触れません。send/receive は例外です。ペアリングコードを発行できるのはサーバーだけで、しかもサインイン済みのアカウントに対してだけなので、送信側は一度 relayium login を実行します。相手から渡されたコードを指定した send は発行を行わないのでログインは不要です。受信側にアカウントは決して必要ありません。",
      },
      {
        q: "厳格な NAT の内側にいて直接経路がない場合はどうなりますか？",
        a: "その場合 Relayium の send/receive は直接接続専用のため失敗します。リレーにはフォールバックしません。magic-wormhole の Transit Relay は暗号化ストリームを運んで転送を完了させることができます。ネットワークの状況にかかわらず確実に動作させたい場合、magic-wormhole は今日その状況に対応します。到達可能なサーバーに対する Relayium の push/pull やデーモン直結も、直接 P2P ホップに依存しないため同様に動作します。",
      },
      {
...
const ko = {
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        "직접 경로가 없을 때: magic-wormhole의 Transit Relay는 암호화된 스트림을 실어 날라 전송이 완료됩니다. Relayium의 send/receive는 직접 연결 전용이라 이 경우 실패합니다.",
        "서버와의 대화: Relayium은 SSH 접근(push/pull) 또는 인증서 고정 TLS 데몬을 재사용합니다. magic-wormhole은 SSH 통합이 없어 양쪽에 설치하고 코드를 공유해야 합니다.",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "계정이 필요한가요?",
        a: "send가 그렇고, 클라우드 up도 그렇습니다. push/pull은 자신의 SSH 접근을 사용하고 데몬 다이렉트는 기기 간 TLS 인증서 고정 신뢰를 사용하므로 둘 다 Relayium 계정을 건드리지 않습니다. send/receive가 예외입니다. 페어링 코드는 서버만, 그것도 로그인된 계정에만 발급할 수 있으므로 보내는 쪽이 relayium login을 한 번 실행합니다. 건네받은 코드를 지정한 send는 발급을 하지 않으므로 로그인이 필요 없습니다. 받는 데는 계정이 전혀 필요 없습니다.",
      },
      {
        q: "엄격한 NAT 뒤에 있어 직접 경로가 없다면 어떻게 되나요?",
        a: "이 경우 Relayium의 send/receive는 직접 연결 전용이라 실패합니다 — 릴레이로 대체되지 않습니다. magic-wormhole의 Transit Relay는 암호화된 스트림을 실어 날라 전송을 완료할 수 있습니다. 네트워크 상황과 무관하게 반드시 성공해야 한다면, magic-wormhole이 오늘 그 상황을 처리합니다. 도달 가능한 서버에 대한 Relayium의 push/pull이나 데몬 다이렉트도 직접 P2P 홉에 의존하지 않으므로 마찬가지로 작동합니다.",
      },
      {
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Mit einem Server sprechen: Relayium nutzt deinen SSH-Zugang (push/pull) oder einen TLS-Daemon mit Pinning; magic-wormhole hat keine SSH-Integration — installiere es auf beiden Seiten und teile einen Code.",
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "relayium push / pull réutilise votre accès SSH existant, donc aucune nouvelle brique à qui faire confiance et aucun code d'appairage à partager. push fonctionne même vers un serveur sans relayium installé, en basculant sur un simple flux tar via la connexion SSH — ce repli n'existe que pour push ; pull a toujours besoin de relayium sur la machine distante, puisqu'il y joue le rôle d'expéditeur.",
    ...
    },
    {
      ...
      bullets: [
        ...
        "Parler à un serveur : Relayium réutilise votre accès SSH (push/pull) ou un daemon TLS avec épinglage ; magic-wormhole n'a aucune intégration SSH — installez-le des deux côtés et partagez un code.",
...
const ar = {
  ...
  sections: [
    ...
    {
      heading: "الموضع الوحيد الذي يكون فيه magic-wormhole أكثر صمودًا: لديه مُرحِّل",
      body: [
    ...
    {
      heading: "SSH و daemon direct: التحدّث إلى خادم تشغّله أصلًا",
      body: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "التحدّث إلى خادم: يعيد Relayium استخدام وصول SSH لديك (push/pull) أو daemon بـ TLS مثبَّت؛ أما magic-wormhole فلا تكامل SSH لديه — ثبّته على الطرفين وشارك رمزًا.",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "هل يحتاج إلى حساب؟",
        a: "‏send نعم، وكذلك up السحابي. يستخدم push/pull وصولك عبر SSH، ويستخدم daemon direct ثقة شهادة TLS المثبَّتة بين أجهزتك، فلا يلمس أيٌّ منهما حساب Relayium. أما send/receive فهو الاستثناء: لا يستطيع إصدار رمز الاقتران إلا الخادم، ولحساب مسجَّل الدخول فقط، لذا يشغّل المُرسِل relayium login مرة واحدة — أما send الذي تمرّر له رمزًا أعطاك إياه غيرك فلا يُصدر شيئًا ولا يحتاج تسجيل دخول. أما الاستقبال فلا يحتاج حسابًا أبدًا.",
      },
      {
  ...
  cta: {
    text: "ثبّت CLI المجاني في Relayium وجرّب push أو sync أو send — مجانًا تمامًا، ونقل قائم على رمز يبدأ بالسرعة نفسها مثل magic-wormhole.",
    button: "احصل على CLI",
    href: "/cli",
  },
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Hablar con un servidor: Relayium reutiliza tu acceso SSH (push/pull) o un daemon sobre TLS con anclaje; magic-wormhole no tiene integración con SSH — instálalo en ambos extremos y comparte un código.",
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Falar com um servidor: o Relayium reutiliza seu acesso SSH (push/pull) ou um daemon sobre TLS com fixação; o magic-wormhole não tem integração com SSH — instale-o nas duas pontas e compartilhe um código.",
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "O send sim, e o up na nuvem também. O push/pull usa seu próprio acesso SSH e o daemon direto usa a confiança de certificado TLS com fixação entre suas máquinas, então nenhum dos dois toca uma conta do Relayium. O send/receive é a exceção: só o servidor pode gerar um código de emparelhamento, e apenas para uma conta com login feito, então quem envia roda relayium login uma vez; um send ao qual você passa um código que lhe deram não gera nenhum e não precisa de login. Receber nunca precisa de conta.",
      },
      {
        q: "E se eu estiver atrás de um NAT rígido e não houver caminho direto?",
        a: "O send/receive do Relayium é somente direto e falhará nesse caso — ele não recorre a um retransmissor. O Transit Relay do magic-wormhole ainda pode carregar o fluxo criptografado e completar a transferência. Se você precisa que funcione não importa como seja a rede, o magic-wormhole cobre esse caso hoje; o push/pull ou o daemon direto do Relayium contra um servidor que você consiga alcançar também funcionam, já que não dependem de um salto P2P direto.",
      },
      {
