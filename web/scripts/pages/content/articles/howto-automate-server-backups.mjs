...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ mirrors deletions (receiver needs serve --allow-delete).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ stays running and re-syncs on change instead of running once from cron.",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ 会镜像删除操作(接收端需要 serve --allow-delete)。",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ 会持续运行,有变化就同步,而不是靠 cron 单次触发。",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ は削除もミラーします(受信側に serve --allow-delete が必要)。",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ は動き続けて変化のたびに同期します。cron による単発実行の代わりに使えます。",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/는 삭제도 미러링합니다(수신 측에 serve --allow-delete 필요).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/는 계속 실행되며 변경 시마다 동기화합니다. cron의 단발 실행 대신 사용할 수 있습니다.",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ spiegelt auch Löschungen (Empfänger braucht serve --allow-delete).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ bleibt laufen und synchronisiert bei jeder Änderung, statt einmalig per cron.",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ met en miroir les suppressions (le récepteur a besoin de serve --allow-delete).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ reste en cours d'exécution et resynchronise à chaque changement, au lieu d'une exécution unique via cron.",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ يعكس عمليات الحذف (يحتاج المستقبِل إلى serve --allow-delete).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ يبقى مستمرًا ويعيد المزامنة عند التغيّر بدلًا من التشغيل مرة واحدة من cron.",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ replica los borrados (el receptor necesita serve --allow-delete).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ se mantiene en ejecución y vuelve a sincronizar al cambiar algo, en lugar de ejecutarse una sola vez desde cron.",
      ],
    },
  ],
  faq: {
...
*/15 * * * * relayium sync -i ~/.ssh/backup_key ~/documents user@backup-server:/srv/backups/ >> ~/relayium-sync.log 2>&1`,
    ...
    },
    {
      ...
      bullets: [
        "relayium sync --delete ./data user@backup-server:/srv/backups/ espelha as exclusões (o receptor precisa de serve --allow-delete).",
        "relayium sync --watch ./data user@backup-server:/srv/backups/ permanece em execução e sincroniza novamente a cada mudança, em vez de rodar uma única vez pelo cron.",
      ],
    },
  ],
  faq: {
