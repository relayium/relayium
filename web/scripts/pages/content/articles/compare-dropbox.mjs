  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "大部分是。软件本身采用 AGPL-3.0 许可、免费开源，完整的协议与代码都在 github.com/relayium/relayium，同一网络内的浏览器传输免费，且不限文件大小。跨网络中继与存储下载链接会占用每个账号都有的额度：每月流量额度，以及同时存放的数据量上限，这两项彼此独立。付费套餐两者都更高；运行你自己的节点，经它中继的字节和存到它上面的上传就不再占用每月流量额度——不过这些文件仍计入存储上限，由 Relayium 转发的下载也照常计入。",
      },
    ],
  },
  cta: {
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "ほとんどの部分は無料です。ソフトウェア自体は AGPL-3.0 ライセンスの無料オープンソースで、プロトコルとコードのすべてが github.com/relayium/relayium にあり、同じネットワーク内のブラウザ転送は無料でサイズ制限もありません。ネットワークをまたぐ中継と保存ダウンロードリンクは、どのアカウントにもある2つの枠を使います。月間転送量の枠と、同時に保存しておける量に対する別のストレージ上限です。有料プランではその両方が引き上げられます。",
      },
    ],
  },
  cta: {
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "대부분은 그렇습니다. 소프트웨어 자체는 AGPL-3.0 라이선스의 무료 오픈소스이며, 프로토콜과 코드 전체가 github.com/relayium/relayium에 있고, 같은 네트워크 안의 브라우저 전송은 무료이며 크기 제한도 없습니다. 네트워크를 넘는 중계와 저장 다운로드 링크는 모든 계정에 주어지는 두 가지 한도를 씁니다. 월간 트래픽 허용량, 그리고 한 번에 저장해 둘 수 있는 양에 대한 별도의 저장 용량 한도입니다. 유료 요금제는 둘 다 높여 줍니다.",
      },
    ],
  },
  cta: {
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "التكلفة والانفتاح: Relayium مفتوح المصدر بموجب رخصة AGPL-3.0 على github.com/relayium/relayium، ويعمل في المتصفّح على Windows وmacOS وLinux وAndroid وiOS دون أي تثبيت. النقل على نفس الشبكة مجاني؛ أما الترحيل عبر الشبكات والروابط المُخزَّنة فيستهلك حصة حركة البيانات الشهرية في خطتك، مع حدّ تخزين منفصل لما تحتفظ به مُخزَّنًا في آنٍ واحد، والخطط المدفوعة ترفع كليهما.",
      ],
    },
  ],
  faq: {
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Coste y apertura: Relayium es de código abierto con licencia AGPL-3.0 en github.com/relayium/relayium, y funciona en el navegador en Windows, macOS, Linux, Android e iOS sin nada que instalar. Las transferencias en la misma red son gratis; la retransmisión entre redes y los enlaces almacenados consumen la franquicia mensual de tráfico de tu plan, con un límite de almacenamiento aparte para lo que guardas almacenado a la vez, y los planes de pago amplían ambos.",
      ],
    },
  ],
  faq: {
