  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "在同一网络下，传输在两台设备之间通过 WebRTC 直连完成。跨网络那条路则是刻意不同的：配对码房间会把 ICE 强制为 relay-only，数据流经由 TURN 中继传递，因此连接一两秒就能建立，而不必花上大约 20 秒去做那些在两个家庭网络之间几乎不可能成功的直连候选探测。无论走哪条路，上面都叠着第二层独立加密：用 X25519 密钥交换协商出密钥，对每个数据块做 AES-256-GCM 加密，这把密钥从不发送给任何服务器——所以中继搬运的始终只是它读不懂的密文。两台设备会显示同一段 6 位校验码（SAS）。通过带外方式核对一致，可确认 X25519 端点公钥未被替换，并检测信令服务或 TURN 中继是否冒充任一端点。这并不证明 TURN 不在网络路径中：跨网络传输时 TURN 仍在数据路径中，但只承载密文。每个文件还会用 SHA-256 做端到端完整性校验。每批最多可发送 1,000 个文件；同一网络下无需账号，而创建配对码以跨网络发送则需要登录——收件的一方始终无需账号。若连接中断，传输还能断点续传而不必从头再来。",
      ],
    },
    {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "같은 네트워크에서는 전송이 두 기기 사이에서 WebRTC로 직접 이루어집니다. 네트워크를 넘는 경로는 일부러 다릅니다. 페어링 코드 방은 ICE를 릴레이 전용으로 고정하므로 스트림이 TURN 릴레이를 지나가며, 가정용 네트워크 사이에서는 거의 성공하지 못하는 직접 후보 검사에 약 20초를 쓰는 대신 1~2초 만에 연결이 올라옵니다. 어느 경로든 그 위에 독립적인 두 번째 암호화 계층이 얹힙니다. X25519 키 교환으로 도출한 키를 블록별 AES-256-GCM에 사용하고, 이 키는 어떤 서버에도 도달하지 않습니다 — 그래서 릴레이가 나르는 것은 스스로 읽을 수 없는 암호문뿐입니다. 두 기기에는 동일한 6자리 검증 코드(짧은 인증 문자열, SAS)가 표시됩니다. 별도 채널로 코드를 비교하면 X25519 엔드포인트 공개 키가 바뀌지 않았음을 확인하고, 시그널링 서비스나 TURN 릴레이가 어느 한쪽 엔드포인트를 사칭했는지 탐지할 수 있습니다. 이는 TURN이 네트워크 경로에 없음을 증명하지 않습니다. 네트워크를 넘을 때 TURN은 여전히 데이터 경로에 있지만 암호문만 운반합니다. 각 파일은 SHA-256 해시로도 종단간 검증됩니다. 한 배치에 최대 1,000개 파일을 보낼 수 있습니다. 같은 네트워크에서는 계정이 필요 없고, 네트워크를 넘어 보내기 위한 페어링 코드를 만들 때는 로그인이 필요합니다 — 받는 쪽은 어느 경우든 계정이 필요 없습니다. 연결이 끊겨도 처음부터가 아니라 이어서 재개할 수 있습니다.",
      ],
    },
    {
