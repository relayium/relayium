  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "当接收方还不在线时,存储下载链接采用的是一套确实不同的零知识方案:你的浏览器会生成一把随机的 AES-256-GCM 密钥,并在任何内容上传之前就用它加密文件。这把密钥从不会发给服务器——它只存在于链接的 URL 片段里,也就是 # 号之后的部分,浏览器从不会传输这部分内容。服务器最终持有的是它无法解密的密文,加上你自己选择的过期时间:1 小时、1 天、3 天、7 天、最长 14 天（上限取决于你的套餐）,或者首次下载完成后即焚。",
