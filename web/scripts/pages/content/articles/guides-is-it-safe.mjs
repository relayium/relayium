  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "受信者がまだオンラインでないとき、保存型のダウンロードリンクは本当に異なるゼロ知識方式を使います。あなたのブラウザは1つのランダムな AES-256-GCM 鍵を生成し、何かがアップロードされる前にそれでファイルを暗号化します。この鍵はサーバーには一切送られません——リンクの URL フラグメント、つまり # の後ろの部分にのみ存在し、ブラウザはこの部分を決して送信しません。結果としてサーバーが持つのは、復号する術のない暗号文と、あなたが選んだ有効期限(1時間、1日、3日、7日、または最初のダウンロード完了後に消去)だけです。",
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "수신자가 아직 온라인이 아닐 때, 저장형 다운로드 링크는 진짜로 다른 영지식 방식을 사용합니다. 당신의 브라우저는 무작위 AES-256-GCM 키를 생성하고, 무언가 업로드되기 전에 그것으로 파일을 암호화합니다. 이 키는 서버로 전혀 전송되지 않습니다——링크의 URL 프래그먼트, 즉 # 뒤의 부분에만 존재하며, 브라우저는 이 부분을 절대 전송하지 않습니다. 결국 서버가 갖게 되는 것은 복호화할 방법이 없는 암호문과, 당신이 선택한 만료 기한(1시간, 1일, 3일, 7일, 또는 첫 다운로드 완료 후 소각)뿐입니다.",
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "حين لا يكون المُستقبِل متصلاً بعد، يستخدم رابط التنزيل المُخزَّن تصميماً مختلفاً فعلاً بمعرفة صفرية: يولّد متصفحك مفتاح AES-256-GCM عشوائياً ويشفّر به الملفات قبل رفع أي شيء. لا يُرسَل هذا المفتاح إلى الخادم أبداً — بل يوجد فقط في جزء عنوان الرابط (URL fragment)، وهو الجزء الذي يلي #، والذي لا ترسله المتصفحات أبداً. ينتهي الأمر بالخادم حاملاً نصاً مُشفَّراً لا سبيل له إلى فك تشفيره، إضافة إلى مدة انتهاء صلاحية تختارها أنت: ساعة واحدة، أو يوم واحد، أو 3 أيام، أو 7 أيام، أو التلاشي بعد أول تنزيل مكتمل.",
