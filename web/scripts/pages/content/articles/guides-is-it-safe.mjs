  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "「零知识」意味着服务器持有的数据,结构上就无法被它自己解密,哪怕是静置状态。",
      ],
    },
    {
      ...
      bullets: [
        ...
        "文件的完整性有没有被校验,这样一个损坏或被改动过的文件不会悄无声息地「正常」抵达?",
        "代码是不是开源、可审查的,还是这些说法只是你必须单凭信任接受的宣传语?",
        "发送或接收是否要求你交出比这次传输真正需要的更多身份信息?",
        "如果有任何内容被存放在服务器上,它有没有一个明确、有限的生命周期——比如过期时间或下载后即焚——而不是无限期地留在那里?",
      ],
    },
    {
      ...
      body: [
        ...
        "当接收方还不在线时,存储下载链接采用的是一套确实不同的零知识方案:你的浏览器会生成一把随机的 AES-256-GCM 密钥,并在任何内容上传之前就用它加密文件。这把密钥从不会发给服务器——它只存在于链接的 URL 片段里,也就是 # 号之后的部分,浏览器从不会传输这部分内容。服务器最终持有的是它无法解密的密文,加上你自己选择的过期时间:1 小时、1 天、3 天、最长 7 天（上限取决于你的套餐）,或者首次下载完成后即焚。",
      ...
      bullets: [
        "同一网络内的传输完全不需要账号。",
        "跨网络用配对码发送,或创建一个存储链接,都需要发送方登录——但接收方永远不需要账号。",
      ],
    },
    {
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "实时模式下不会——文件直接在两个浏览器之间流动,什么都不会被存储。对于存储链接,服务器只持有它无法读取的加密密文,直到链接过期,或者如果你选了「首次下载后即焚」,下载一次之后即被清除。",
      },
      {
        q: "发送或接收文件需要账号吗?",
        a: "在同一网络内,双方都不需要账号。跨网络用配对码发送,或创建一个存储链接,需要发送方登录——但无论用哪种方式,接收方都从不需要账号。",
      },
    ],
  },
  cta: {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "受信者がまだオンラインでないとき、保存型のダウンロードリンクは本当に異なるゼロ知識方式を使います。あなたのブラウザは1つのランダムな AES-256-GCM 鍵を生成し、何かがアップロードされる前にそれでファイルを暗号化します。この鍵はサーバーには一切送られません——リンクの URL フラグメント、つまり # の後ろの部分にのみ存在し、ブラウザはこの部分を決して送信しません。結果としてサーバーが持つのは、復号する術のない暗号文と、あなたが選んだ有効期限(1時間、1日、3日、最長7日（プランによる）、または最初のダウンロード完了後に消去)だけです。",
        "2台のデバイス間で直接接続ができない場合、暗号化されたストリームは失敗する代わりに TURN 中継にフォールバックします——その中継もやはり暗号文しか見ることがなく、読める形のファイルを見ることは決してありません。",
        "これらはどれも、あなたが主張を鵜呑みにする必要はありません。Relayium のクライアントとサーバーのコードは MIT ライセンスのもとオープンソースで公開されており、宣伝を信じるのではなく、誰でも読んで監査できます。",
      ],
      bullets: [
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "수신자가 아직 온라인이 아닐 때, 저장형 다운로드 링크는 진짜로 다른 영지식 방식을 사용합니다. 당신의 브라우저는 무작위 AES-256-GCM 키를 생성하고, 무언가 업로드되기 전에 그것으로 파일을 암호화합니다. 이 키는 서버로 전혀 전송되지 않습니다——링크의 URL 프래그먼트, 즉 # 뒤의 부분에만 존재하며, 브라우저는 이 부분을 절대 전송하지 않습니다. 결국 서버가 갖게 되는 것은 복호화할 방법이 없는 암호문과, 당신이 선택한 만료 기한(1시간, 1일, 3일, 최대 7일(요금제에 따라 다름), 또는 첫 다운로드 완료 후 소각)뿐입니다.",
        "두 기기 사이에 직접 연결이 불가능할 때, 암호화된 스트림은 실패하는 대신 TURN 중계로 대체됩니다——그리고 그 중계 역시 언제나 암호문만 보며, 읽을 수 있는 파일을 보는 일은 결코 없습니다.",
        "이 중 어느 것도 당신이 주장을 그냥 믿어야만 하는 것은 없습니다. Relayium의 클라이언트와 서버 코드는 MIT 라이선스로 공개되어 있어, 홍보 문구를 믿는 대신 누구나 읽고 감사할 수 있습니다.",
      ],
      bullets: [
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "Quand le destinataire n'est pas encore en ligne, un lien de téléchargement stocké utilise un mécanisme réellement différent, à connaissance nulle : votre navigateur génère une clé AES-256-GCM aléatoire et l'utilise pour chiffrer les fichiers avant tout envoi. Cette clé n'est jamais envoyée au serveur — elle ne vit que dans le fragment d'URL du lien, la partie après le #, que les navigateurs ne transmettent jamais. Le serveur finit par détenir uniquement du chiffré qu'il n'a aucun moyen de déchiffrer, plus une expiration que vous choisissez : 1 heure, 1 jour, 3 jours, jusqu'à 7 jours selon votre offre, ou effacement après le premier téléchargement complet.",
        "Quand une connexion directe entre deux appareils n'est pas possible, le flux chiffré bascule vers un relais TURN plutôt que d'échouer — et ce relais aussi ne voit toujours que du chiffré, jamais un fichier lisible.",
        "Rien de tout cela ne vous demande de croire une affirmation sur parole : le code client et serveur de Relayium est open source sous licence MIT, et peut donc être lu et audité plutôt que simplement cru.",
      ],
      bullets: [
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "حين لا يكون المُستقبِل متصلاً بعد، يستخدم رابط التنزيل المُخزَّن تصميماً مختلفاً فعلاً بمعرفة صفرية: يولّد متصفحك مفتاح AES-256-GCM عشوائياً ويشفّر به الملفات قبل رفع أي شيء. لا يُرسَل هذا المفتاح إلى الخادم أبداً — بل يوجد فقط في جزء عنوان الرابط (URL fragment)، وهو الجزء الذي يلي #، والذي لا ترسله المتصفحات أبداً. ينتهي الأمر بالخادم حاملاً نصاً مُشفَّراً لا سبيل له إلى فك تشفيره، إضافة إلى مدة انتهاء صلاحية تختارها أنت: ساعة واحدة، أو يوم واحد، أو 3 أيام، أو حتى 7 أيام حسب خطتك، أو التلاشي بعد أول تنزيل مكتمل.",
        "حين يتعذّر الاتصال المباشر بين جهازين، يلجأ التدفق المُشفَّر إلى مُرحِّل TURN بدلاً من الفشل — وهذا المُرحِّل أيضاً لا يرى سوى النص المُشفَّر، ولا يرى أبداً ملفاً قابلاً للقراءة.",
        "لا يطلب أيٌّ من هذا أن تصدّق ادّعاءً على عِلّاته: شيفرة عميل Relayium وخادمه مفتوحة المصدر بموجب رخصة MIT، فيمكن قراءتها وتدقيقها بدلاً من تصديقها على عِلّاته.",
      ],
      bullets: [
