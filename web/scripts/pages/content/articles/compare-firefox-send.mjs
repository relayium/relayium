...
const en = {
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "Do links expire?",
        a: "Yes. Choose 1 hour, 1 day, 3 days, or 7 days, or set the link to burn after the first complete download. After that, the stored ciphertext is deleted and the link no longer works.",
      },
      {
        q: "Is Relayium free?",
...
const zh = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Relayium 的异步模式围绕同样的体验构建。你选择文件后，浏览器会在本地生成一个随机的 AES-256-GCM 密钥，用它加密文件并上传密文。服务器从不会看到密钥，也无法解密它所存储的内容——这是真正的零知识，而不只是「静态加密」。",
        "解密密钥只存在于链接本身之中，也就是 # 之后的 URL 片段，浏览器从不会把这部分发送给任何服务器。拿到链接的人下载时无需账号——直接打开链接，浏览器就会取回密文并用片段中的密钥在本地解密。这正是 Firefox Send 当年的核心承诺，完整保留：一个自带密钥、托管方读不懂的链接。",
      ],
    },
    {
      heading: "到期、首次下载即焚，以及坦诚说明的限制",
      body: [
        "和 Firefox Send 一样，Relayium 的每个链接都有你自己选择的有效期：1 小时、1 天、3 天或 7 天，或者设置为「首次完整下载后即焚」，收件方一拿到文件链接就会失效。链接到期或焚毁后，存储的密文会被删除，链接随之失效。",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "链接会过期吗？",
        a: "会。可选择 1 小时、1 天、3 天或 7 天，或设置为首次完整下载后即焚。之后，存储的密文会被删除，链接不再可用。",
      },
      {
        q: "Relayium 免费吗？",
...
const ja = {
  ...
  updatedLabel: "最終更新",
  lead: [
    "Firefox Send は Mozilla によるプライベートなファイル共有の試みでした。ファイルを入れると暗号化されたリンクが得られ、期限切れや1回のダウンロード後の自動消去を設定できました。多くの人がこれを気に入り、何年経った今も探し続けています。もう戻ってきません——Mozilla は2020年にこのサービスを終了しました——ですが、それが満たしていたニーズは今も現実に存在します。",
    "Relayium の非同期（保存リンク）モードは同じ発想を引き継いでいます。ブラウザで暗号化し、リンクを共有し、期限切れや消去を見届ける。この記事は正直な比較です。Firefox Send が何をしていたか、Relayium はどれほど近いか、そして Relayium がリアルタイムの P2P 転送、CLI、セルフホストでどこまで先へ進んでいるかを見ていきます。",
  ],
  sections: [
    {
    ...
    },
    {
      ...
      body: [
        "Firefox Send と同様に、Relayium のすべてのリンクには自分で選ぶ有効期間があります。1時間、1日、3日、7日、あるいは最初の完全なダウンロードの後に消去するよう設定することもでき、その瞬間に受信者が手にしたファイルとともにリンクが消えます。リンクが期限切れになるか消去されると、保存された暗号文は削除され、リンクは機能しなくなります。",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "リンクは期限切れになりますか？",
        a: "はい。1時間、1日、3日、7日から選ぶか、最初の完全なダウンロード後に消去するよう設定できます。その後、保存された暗号文は削除され、リンクは使えなくなります。",
      },
      {
        q: "Relayium は無料ですか？",
...
const ko = {
  ...
  lead: [
    "Firefox Send는 Mozilla가 시도한 사적인 파일 공유 방식이었습니다. 파일을 넣으면 암호화된 링크를 받고, 만료 시점이나 첫 다운로드 후 자동 소멸을 설정할 수 있었습니다. 많은 사람이 이를 좋아했고 몇 년이 지난 지금도 여전히 찾고 있습니다. 다시 돌아오지는 않습니다——Mozilla는 2020년에 이 서비스를 중단했습니다——하지만 그것이 채워주던 필요는 지금도 실재합니다.",
    "Relayium의 비동기(저장 링크) 모드는 같은 발상을 이어받습니다. 브라우저에서 암호화하고, 링크를 공유하고, 만료되거나 소각되는 것을 지켜보세요. 이 글은 정직한 비교입니다——Firefox Send가 무엇을 했는지, Relayium이 얼마나 가깝게 닮았는지, 그리고 Relayium이 실시간 P2P 전송, CLI, 셀프호스팅으로 어디까지 더 나아가는지를 다룹니다.",
  ],
  sections: [
    {
    ...
    },
    {
      ...
      body: [
        "Firefox Send와 마찬가지로 Relayium의 모든 링크는 직접 고르는 수명을 가집니다: 1시간, 1일, 3일, 7일 중 선택하거나, 첫 완전한 다운로드 후 소각되도록 설정할 수 있어 받는 쪽이 파일을 손에 넣는 순간 링크가 사라집니다. 링크가 만료되거나 소각되면 저장된 암호문이 삭제되고 링크는 더 이상 작동하지 않습니다.",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "링크가 만료되나요?",
        a: "네. 1시간, 1일, 3일, 7일 중에서 고르거나 첫 완전한 다운로드 후 소각되도록 설정할 수 있습니다. 그 후에는 저장된 암호문이 삭제되고 링크가 더 이상 작동하지 않습니다.",
      },
      {
        q: "Relayium은 무료인가요?",
...
const de = {
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "Laufen Links ab?",
        a: "Ja. Wähle 1 Stunde, 1 Tag, 3 Tage oder 7 Tage, oder stelle den Link so ein, dass er nach dem ersten vollständigen Download vernichtet wird. Danach wird der gespeicherte Chiffretext gelöscht und der Link funktioniert nicht mehr.",
      },
      {
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "Oui. Choisissez 1 heure, 1 jour, 3 jours ou 7 jours, ou réglez le lien pour qu'il se détruise après le premier téléchargement complet. Ensuite, le texte chiffré stocké est supprimé et le lien ne fonctionne plus.",
      },
      {
...
const ar = {
  ...
  lead: [
    "كان Firefox Send رؤية Mozilla لمشاركة الملفات الخاصة: تضع ملفًا، فتحصل على رابط مشفّر، وتضبطه لينتهي أو يُدمَّر ذاتيًا بعد تنزيل واحد. أحبّه الناس ولا يزالون يبحثون عنه بعد سنوات. لن يعود — فقد أوقفت Mozilla الخدمة في 2020 — لكن الحاجة التي كان يلبّيها لا تزال قائمة فعلًا.",
    "يلتقط وضع Relayium غير المتزامن (رابط مُخزَّن) الفكرة ذاتها: التشفير في المتصفح، ومشاركة رابط، ومراقبته وهو ينتهي أو يحترق. هذه المقالة مقارنة صادقة — ماذا كان Firefox Send يفعل، وكم يقترب منه Relayium، وأين يذهب Relayium أبعد من ذلك بالنقل الفوري من الند للند، وواجهة سطر الأوامر CLI، والاستضافة الذاتية.",
  ],
  sections: [
    {
    ...
    },
    {
      ...
      body: [
        "مثل Firefox Send، لكل رابط في Relayium عمر تختاره أنت: ساعة واحدة أو يوم واحد أو 3 أيام أو 7 أيام، أو يمكنك ضبطه ليحترق بعد أول تنزيل كامل فيزول لحظة حصول المُستقبِل على الملف. بمجرد أن ينتهي الرابط أو يحترق، يُحذف النص المُشفَّر المُخزَّن ويتوقف الرابط عن العمل.",
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "هل تنتهي الروابط؟",
        a: "نعم. اختر ساعة واحدة أو يومًا واحدًا أو 3 أيام أو 7 أيام، أو اضبط الرابط ليحترق بعد أول تنزيل كامل. بعد ذلك يُحذف النص المُشفَّر المُخزَّن ولا يعمل الرابط بعدها.",
      },
      {
        q: "هل Relayium مجاني؟",
