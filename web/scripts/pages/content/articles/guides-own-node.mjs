...
enabled`,
    ...
    },
    {
      ...
      body: [
        ...
        "그 계정 주위로 유닛은 systemd 샌드박스를 두릅니다. 모든 줄이 “노드 프로세스가 장악당했을 때 공격자가 다음으로 손을 뻗을 대상”을 겨냥합니다. 감춰 둔 것은 없습니다. 설치 후 cat /etc/systemd/system/relayium-node.service로 유닛 전체를 읽어 보세요.",
      ],
      table: {
    ...
    },
    {
      ...
      bullets: [
        ...
        "저장 디렉터리 자체는 RELAYIUM_NODE_PURGE_STORAGE=1을 함께 넘기지 않는 한 남습니다. 환경 변수는 | sudo sh를 넘지 못하므로 sudo env RELAYIUM_NODE_PURGE_STORAGE=1 sh uninstall-node.sh 형태로 넘기세요.",
        "그다음 systemctl status relayium-node는 not-found여야 하고 relayium-node 타이머도 보이지 않아야 합니다. 나중에 다시 설치하면 계정에 생기는 것은 완전히 새 노드입니다. 제거가 노드의 신원 파일을 지우므로, 재설치는 옛 노드를 되살리는 것이 아니라 새로 등록하는 것입니다.",
      ],
    },
    {
      heading: "잘 안 될 때",
