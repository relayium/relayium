...
const zh = {
  ...
  sections: [
    ...
    },
    {
      heading: "扫码之后发生了什么",
      body: [
        "扫码只是让两台设备进到同一个房间；传输本身和其他任何配对码会话完全一样。创建配对码的一方拥有这个房间，扫码的一方加入进来——但这说的是「谁加入谁」，不是方向：两台都进来之后，哪一侧都可以当发送方，而每一批送过来的文件仍然要在接收端按「接收」才会开始。配对码把你们放进一个单独的两台设备房间，而不是同网络的设备列表；但房间里的界面就是两个够新的浏览器在哪里都会打开的那个共享工作区：对端卡片上一个动作，然后一条加密连接同时承载文件和消息。配对码真正改变的是路径——它按设计只走中继，而这条中继链路有一个有限的存活期，工作区会在到点之前提醒你。两边浏览器用 X25519 密钥交换协商出共享密钥，用 AES-256-GCM 对每个数据块加密，再把这些数据块通过架在 TURN 中继上的 WebRTC 通道发出去。",
...
const ja = {
  ...
  sections: [
    ...
    },
    {
      heading: "スキャン後に起きること",
      body: [
        "スキャンは2台の端末を同じルームに入れるだけで、転送そのものは他のペアリングコードのセッションと変わりません。コードを作った側がルームの持ち主で、スキャンした側が参加者ですが、それは待ち合わせの話であって向きの話ではありません。2台が入ってしまえば送る側はどちらでもよく、届くファイルの一式は受信側の画面の「受信」で必ずゲートされます。ペアリングコードが入れてくれるのは、同一ネットワークの端末一覧ではなく2台だけの別のルームです。ただしそのルームの中身は、十分に新しいブラウザ同士がどこでも開くのと同じ共有ワークスペースで、相手のカードの操作は一つ、そのあとは1本の暗号化接続がファイルとメッセージをまとめて運びます。ペアリングコードが実際に変えるのは経路です。設計上リレー経由のみで、そのリレー接続には有限の寿命があり、ワークスペースは到達前に知らせます。両端末は X25519 鍵交換で共有鍵を導出し、各チャンクを AES-256-GCM で暗号化したうえで、TURN リレー上に張られた WebRTC チャネルで送り出します。",
...
const ko = {
  ...
  sections: [
    ...
    },
    {
      heading: "스캔 후 일어나는 일",
      body: [
        "스캔은 두 기기를 같은 방에 넣어줄 뿐, 전송 자체는 다른 페어링 코드 세션과 다르지 않습니다. 코드를 만든 쪽이 방의 주인이고 스캔한 쪽이 참여자이지만, 그것은 만남에 대한 이야기이지 방향에 대한 이야기가 아닙니다. 둘 다 들어오고 나면 보내는 쪽은 어느 쪽이든 될 수 있고, 들어오는 파일 묶음은 여전히 받는 화면의 “받기”로 통제됩니다. 페어링 코드는 같은 네트워크의 기기 목록이 아니라 두 기기만의 별도 방으로 들여보냅니다. 다만 그 방 안의 화면은 충분히 최신인 브라우저 둘이 어디서든 여는 것과 같은 공유 작업 공간입니다. 상대 카드에서 동작 하나, 그다음에는 하나의 암호화된 연결이 파일과 메시지를 함께 나릅니다. 페어링 코드가 실제로 바꾸는 것은 경로입니다. 설계상 릴레이만 사용하고, 그 릴레이 연결에는 유한한 수명이 있으며 작업 공간이 그 전에 알려 줍니다. 두 기기는 X25519 키 교환으로 공유 키를 도출하고 각 청크를 AES-256-GCM으로 암호화한 뒤, TURN 릴레이 위에 놓인 WebRTC 채널로 내보냅니다.",
...
const de = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Scannen bringt die beiden Geräte lediglich in denselben Raum; die Übertragung selbst unterscheidet sich nicht von jeder anderen Pairing-Code-Sitzung. Wer den Code erstellt hat, dem gehört der Raum, und wer gescannt hat, ist beigetreten — das ist eine Aussage über das Treffen, nicht über die Richtung: Sind beide drin, kann jede Seite die sendende sein, und jeder eingehende Stapel bleibt durch „Annehmen“ auf dem empfangenden Bildschirm abgesichert. Ein Pairing-Code setzt euch in einen separaten Raum für zwei Geräte statt in die Geräteliste des eigenen Netzes. Die Oberfläche in diesem Raum ist aber derselbe gemeinsame Arbeitsbereich, den zwei aktuelle Browser überall öffnen: eine Aktion auf der Karte der Gegenstelle, dann eine verschlüsselte Verbindung für Dateien und Nachrichten zugleich. Was der Code sehr wohl ändert, ist der Weg: Er läuft von Grund auf nur über Relay, und diese Relay-Verbindung hat eine begrenzte Lebensdauer, auf die der Arbeitsbereich vorher hinweist. Die beiden Browser leiten mit einem X25519-Schlüsselaustausch einen gemeinsamen Schlüssel ab, verschlüsseln jeden Block mit AES-256-GCM und schicken ihn über einen WebRTC-Kanal, der auf einem TURN-Relay aufsetzt.",
...
const fr = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Scanner ne fait qu'amener les deux appareils dans la même salle ; le transfert lui-même ne diffère en rien de n'importe quelle autre session par code d'appairage. Celui qui a créé le code possède la salle et celui qui a scanné l'a rejointe, mais cela décrit le rendez-vous, pas le sens du transfert : une fois tous les deux entrés, l'un comme l'autre peut être l'expéditeur, et chaque lot entrant reste conditionné à « Accepter » sur l'écran qui reçoit. Un code d'appairage vous place dans une salle séparée à deux appareils plutôt que dans la liste des appareils du même réseau, mais la surface à l'intérieur de cette salle est le même espace de travail partagé que deux navigateurs à jour ouvrent partout : une action sur la carte du pair, puis une connexion chiffrée qui porte ensemble les fichiers et les messages. Ce que le code change vraiment, c'est le chemin : il passe uniquement par le relais, par conception, et cette liaison relayée a une durée de vie bornée que l'espace de travail signale avant qu'elle ne soit atteinte. Les deux navigateurs dérivent une clé partagée via un échange de clés X25519, chiffrent chaque bloc avec AES-256-GCM, puis les envoient sur un canal WebRTC porté par un relais TURN.",
...
const ar = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "المسح يُدخل الجهازين إلى الغرفة نفسها فحسب؛ أما النقل نفسه فلا يتغير عن أي جلسة رمز اقتران أخرى. ومن أنشأ الرمز يملك الغرفة ومن مسحه انضم إليها، لكن هذا حديث عن اللقاء لا عن الاتجاه: فبعد دخولكما معًا، يمكن لأي طرف أن يكون المُرسِل، وتظل كل دفعة واردة محكومة بزر «قبول» على شاشة الاستقبال. رمز الاقتران يضعكما في غرفة منفصلة لجهازين بدل قائمة أجهزة الشبكة نفسها، غير أنّ الواجهة داخل تلك الغرفة هي مساحة العمل المشتركة نفسها التي يفتحها متصفحان حديثان في أي مكان: إجراء واحد على بطاقة الطرف الآخر، ثم اتصال مشفَّر واحد يحمل الملفات والرسائل معًا. أما ما يغيّره الرمز فعلًا فهو المسار: فهو بحكم التصميم عبر المُرحِّل وحده، ولتلك الوصلة المُرحَّلة عمر محدود تنبّه إليه مساحة العمل قبل بلوغه. يشتقّ المتصفحان مفتاحًا مشتركًا بتبادل مفاتيح X25519، ويشفّران كل جزء بـ AES-256-GCM، ثم يرسلانه عبر قناة WebRTC محمولة على مُرحِّل TURN.",
...
const pt = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Escanear apenas coloca os dois dispositivos na mesma sala; a transferência em si não muda em relação a qualquer outra sessão com código de emparelhamento. Quem criou o código é dono da sala e quem escaneou entrou nela, mas isso fala do encontro, não do sentido: com os dois lá dentro, qualquer um dos lados pode ser o que envia, e todo lote que chega continua passando pelo “Aceitar” na tela de quem recebe. Um código de emparelhamento coloca os dois dispositivos numa sala separada em vez da lista de dispositivos da mesma rede, mas a superfície dentro dessa sala é a mesma área de trabalho compartilhada que dois navegadores atualizados abrem em qualquer lugar: uma ação no cartão do par e depois uma conexão criptografada que leva juntos os arquivos e as mensagens. O que o código de fato muda é o caminho: ela vai só por retransmissor por decisão de projeto, e esse enlace retransmitido tem uma vida limitada que a área de trabalho avisa antes de ser atingida. Os dois navegadores derivam uma chave compartilhada com uma troca de chaves X25519, criptografam cada bloco com AES-256-GCM e o enviam por um canal WebRTC apoiado em um retransmissor TURN.",
