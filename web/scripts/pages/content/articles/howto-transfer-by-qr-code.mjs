  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "打开「高级验证」（默认关闭）后，两块屏幕会显示同一段六位数字校验码（SAS）——扫一眼两边，确认一致后再信任这条连接。校验码一致能确认密钥未被替换：服务器或中继没有冒充任一端，也没有终止应用层端到端加密。配对码传输一律走这条中继路径，而不是先去试直连：跨越两个网络时直连路径通常根本不存在，先探一遍只会让连接卡住二十秒左右，最后仍旧落到中继上。中继只转发密文——它没有密钥，永远看不到你的文件内容。每个文件都会用 SHA-256 哈希做端到端完整性校验，中途断开也能续传而不必重来。",
      ],
    },
    {
      heading: "扫码失败怎么办",
