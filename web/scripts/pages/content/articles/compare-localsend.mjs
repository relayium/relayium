...
const ar = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "LocalSend مخصص للشبكة المحلية فقط بحكم التصميم — ليس لديه أي وسيلة للوصول إلى جهاز على شبكة مختلفة أو عبر الإنترنت. يضيف Relayium ذلك: أنشئ رمز اقتران (أو شارك رابط الانضمام الذي يولّده)، فيتصل الطرف الآخر من أي مكان، من الند للند مباشرةً كلما أمكن.",
        "يتطلب إنشاء رمز اقتران عبر الشبكات تسجيل دخول المُرسِل؛ أما الشخص المُستقبِل فلا يحتاج إلى حساب أبدًا. كل عملية نقل فوري — على الشبكة المحلية أو عبر الشبكات — مشفّرة من الطرف إلى الطرف على مستوى التطبيق: يشتقّ تبادل مفاتيح X25519 مفتاحًا يُستخدم لـ AES-256-GCM، ويتيح رمز SAS من 6 أرقام للطرفين تأكيد عدم وجود خادم في المنتصف، ويُتحقَّق من كل ملف بتجزئة SHA-256. وعندما يتعذّر الاتصال المباشر، يحمل مُرحِّل TURN مشفّر نصًا مُشفَّرًا لا يستطيع قراءته.",
        "لأي شخص يكتب سكربتات لنقل الملفات إلى خادم، توجد أيضًا واجهة سطر أوامر — push/pull عبر SSH أو مباشرةً عبر الخفيّ (daemon)، ومزامنة تزايدية للمجلدات، وإرسال/استقبال برمز اقتران — وهو ما لا يقدّمه تطبيق مقتصر على الشبكة المحلية.",
      ],
    },
    {
    ...
    },
    {
      ...
      bullets: [
        ...
        "مدى الشبكة: LocalSend مخصص للشبكة المحلية فقط؛ أما Relayium فيتصل أيضًا عبر الشبكات المختلفة برمز اقتران.",
        ...
        "الحسابات: LocalSend لا يحتاج إلى حساب أبدًا؛ ووضع الشبكة المحلية في Relayium كذلك لا يحتاج إليه — المُرسِل وحده يسجّل الدخول لعمليات النقل عبر الشبكات.",
  ...
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "نعم. أنشئ رمز اقتران (أو شارك رابط الانضمام الذي يولّده) فيتصل الجهاز الآخر من أي مكان، لا يزال من الند للند مباشرةً كلما أمكن. يحتاج المُرسِل إلى تسجيل الدخول لهذا؛ أما المُستقبِل فلا يحتاج إليه أبدًا.",
      },
      {
      ...
      },
      {
        ...
        a: "كلاهما يُبقي عمليات النقل على نفس الشبكة بعيدًا عن خادم عام. يؤمّن LocalSend ناقله المحلي. ويضيف Relayium قناة تشفير مستقلة على مستوى التطبيق مع رمز SAS يمكن للطرفين التحقق منه، على الشبكة المحلية أو عبر الشبكات.",
      },
    ],
  },
  cta: {
