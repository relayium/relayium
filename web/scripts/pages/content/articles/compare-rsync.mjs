  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Where Relayium sync wins: no SSH server required (daemon direct), pinned TLS with fingerprint authentication, real-time re-sync with --watch, and the direct modes — sync, push/pull and daemon direct — are free with nothing metered; sync needs no account at all.",
      ],
    },
  ],
  faq: {
    ...
    items: [
      ...
      {
        q: "Is it free?",
        a: "For everything this comparison covers, yes. The Relayium CLI is free and open source, and sync, push/pull and daemon-direct are direct transfers with nothing metered and no account needed. send needs an account so the server can mint its pairing code. Only cloud up touches hosted storage, where your plan's storage limit applies.",
      },
    ],
  },
  cta: {
  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        "Relayium sync 的优势：不需要 SSH 服务（daemon 直连）、带指纹认证的证书固定 TLS、用 --watch 实时重新同步，而且直连模式——sync、push/pull 与 daemon 直连——都免费、不计量；sync 本身完全不需要账号。",
      ],
    },
  ],
  faq: {
    ...
    items: [
      ...
      {
        q: "这是免费的吗？",
        a: "就本文对比涉及的部分而言，是的。Relayium CLI 免费且开源，sync、push/pull 和 daemon 直连都是直连传输，不计量，也不需要账号。send 需要账号，好让服务器签发配对码。只有云端 up 会用到托管存储，受套餐的存储上限约束。",
      },
    ],
  },
  cta: {
