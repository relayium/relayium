  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        ...
        "阅后即焚意味着链接会在收件人下载完成的那一刻立即失效——文件比较敏感、你只想让一个人看一次、事后不留尾巴时很有用。无论选哪种，收件人在下载页都能看到实时倒计时，清楚还剩多少时间（或者还能不能下载）。",
      ],
    ...
    },
    {
      ...
      body: [
        ...
        "这个设计有几个值得了解的真实取舍。链接本身是密钥的唯一副本，一旦弄丢就没有找回密码这回事——服务器上的数据会变得永久不可读。创建链接需要发送方处于登录状态，而且每个链接在被删除或过期之前，都会占用你账号的存储配额。这里的「零知识」也有必要说清楚：密文确实在创建到下载之间存放在 Relayium 的服务器上——只是少了那把从未离开过你浏览器的密钥，谁都读不懂它。",
      ],
    },
  ],
  faq: {
    ...
    items: [
      ...
      },
      {
        ...
        a: "不能。你的浏览器会在上传前用一把随机生成的 AES-256-GCM 密钥加密文件，这把密钥只存在于 URL 片段里，永远不会到达服务器。服务器保存的是它无法解密的密文——这就是零知识设计。",
      },
      {
        q: "链接能保留多久？",
        a: "由你在创建时选择：1 小时、1 天、3 天、7 天或最长 14 天（可选范围取决于套餐）；也可以不设固定时长，改成首次完整下载后即焚。过了这个点，链接就不再有效。",
      },
      {
        q: "开了阅后即焚，文件真的会消失吗？",
        a: "阅后即焚就是为「只下载一次」设计的：收件人一完成下载，之后任何人再打开这个链接都不会生效，所以它没法被人悄悄再看一次。",
      },
    ],
  },
  cta: {
