...
const zh = {
  ...
  lead: [
    ...
    "但很多人用 Drive 做的其实是一次性的「交接」——把一段视频发给朋友、把合同发给客户、把数据集发给同事——而不是持久存储。对这类具体任务来说，把一份副本上传到 Google 的服务器、再配置谁能看到它，比任务本身需要的步骤要多。Relayium 正是为这种交接而生：文件直接在两台设备之间传输，或通过一个只有接收方能解密的链接发送。",
  ],
  sections: [
    {
    ...
    },
    {
      ...
      body: [
        ...
        "对常见场景，Relayium 干脆跳过了存储这一步。它的实时模式通过端到端加密的点对点连接，在发送方和接收方的设备之间传输字节——文件本身根本不会存到任何服务器上。当接收方不在线、你确实需要一个链接时，存储链接模式保留了 Drive 所没有的零知识特性：你的浏览器会在上传前用随机生成的 AES-256-GCM 密钥加密文件，而这个密钥只存在于 URL 片段中（# 之后的部分，浏览器从不把它发给服务器）。Google 的服务器持有并且在技术上可以读取你 Drive 里的文件；而 Relayium 的服务器对于存储链接，只保存它无法解密的密文。",
      ],
    },
    {
...
const ja = {
  ...
  lead: [
    ...
    "しかし多くの人が Drive を使っている場面は、実は一度きりの受け渡しです。友人に動画を送る、クライアントに契約書を送る、同僚にデータセットを送るといったことであり、永続的な保存ではありません。その特定の作業のためだけに、コピーを Google のサーバーへアップロードし、誰が見られるかを設定するのは、必要以上の手間です。Relayium はまさにその受け渡しのために作られています。ファイルを2台の端末間で直接送るか、受信者だけが復号できるリンクで送ります。",
  ],
  sections: [
    {
    ...
    items: [
      {
        ...
        a: "リアルタイムモードでは保存しません。ファイルは端末間を直接移動し、サーバーには一切保存されません。ダウンロードリンクを作成した場合、サーバーは何かを保存しますが、それは復号できないゼロ知識の暗号文だけです。リンクは失効するか（1時間/1日/3日/7日、プランによる）、最初のダウンロード後に削除されます。",
      },
      {
        q: "アカウントは必要ですか？",
...
const ko = {
  ...
  lead: [
    ...
    "하지만 많은 사람이 Drive로 하는 일은 실제로는 일회성 전달입니다. 친구에게 동영상을 보내거나, 고객에게 계약서를 보내거나, 동료에게 데이터셋을 보내는 일이지 영구 저장이 아닙니다. 그 특정 작업만 놓고 보면, 사본을 Google 서버에 업로드하고 누가 볼 수 있는지 설정하는 것은 작업에 필요한 것보다 더 많은 절차입니다. Relayium은 바로 그 전달을 위해 만들어졌습니다. 파일을 두 기기 사이에서 직접 보내거나, 받는 사람만 복호화할 수 있는 링크로 보냅니다.",
  ],
  sections: [
    {
    ...
    },
    {
      ...
      body: [
        ...
        "Relayium은 흔한 경우에 이 저장 단계를 건너뜁니다. 실시간 모드는 종단간 암호화된 P2P 연결을 통해 보내는 사람과 받는 사람의 기기 사이로 바이트를 옮깁니다. 파일 자체는 서버에 전혀 저장되지 않습니다. 받는 사람이 오프라인이라 링크가 정말로 필요할 때는, 저장 링크 모드가 Drive에는 없는 영지식 속성을 유지합니다. 브라우저가 업로드 전에 무작위로 생성한 AES-256-GCM 키로 파일을 암호화하고, 그 키는 URL 프래그먼트, 즉 # 뒤의 브라우저가 서버로 절대 보내지 않는 부분에만 존재합니다. Google의 서버는 Drive 파일을 보관하며 기술적으로 읽을 수 있습니다. 반면 Relayium의 서버는 저장 링크에 대해 복호화할 수 없는 암호문만 보관합니다.",
      ],
    },
    {
    ...
    items: [
      {
        ...
        a: "실시간 모드에서는 아닙니다. 파일이 기기 사이에서 직접 이동하며 서버에 전혀 저장되지 않습니다. 다운로드 링크를 만들면 서버가 무언가를 저장하지만, 그것은 복호화할 수 없는 영지식 암호문뿐입니다. 링크는 만료되거나(1시간/1일/3일/7일, 요금제에 따라 다름) 첫 다운로드 후 삭제됩니다.",
      },
      {
        q: "계정이 필요한가요?",
...
const de = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Für den klassischen Fall — beide sind gerade online — sendet die direkte Echtzeitübertragung bis zu 1.000 Dateien in einem Durchgang direkt zwischen den Geräten, ohne Hochladeschritt und ohne dass zwischendurch irgendwo etwas gespeichert wird. Beide Seiten sehen einen übereinstimmenden sechsstelligen Verifizierungscode (SAS), um einen Man-in-the-Middle auszuschließen, und jede Datei wird per SHA-256-Hash Ende-zu-Ende geprüft; bricht die Verbindung ab, wird die Übertragung fortgesetzt statt neu gestartet.",
...
const ar = {
  ...
  description:
    "مقارنة منصفة لعملية تسليم ملف لمرة واحدة: Google Drive تخزين دائم ممتاز، لكن الإرسال يعني الرفع وإدارة الأذونات. يرسل Relayium من الند للند أو عبر رابط ذاتي الانتهاء بمعرفة صفرية.",
  ...
  lead: [
    ...
    "لكن كثيرًا مما يستخدم الناس Drive لأجله هو في الحقيقة تسليم لمرة واحدة — إرسال مقطع فيديو إلى صديق، أو عقد إلى عميل، أو مجموعة بيانات إلى زميل — لا تخزين دائم. لتلك المهمة تحديدًا، فإن رفع نسخة إلى خوادم Google ثم ضبط من يمكنه رؤيتها آليةٌ أكثر مما تتطلبه المهمة. بُني Relayium لتلك التسليمة بالضبط: يرسل الملف مباشرةً بين الجهازين، أو عبر رابط لا يستطيع فكّ تشفيره سوى المُستقبِل.",
  ],
  sections: [
    {
    ...
    items: [
      {
        ...
        a: "ليس في الوضع الفوري — ينتقل الملف مباشرةً بين الأجهزة ولا يُخزَّن أبدًا على خادم. إن أنشأت رابط تنزيل، فإن الخادم يخزّن شيئًا بالفعل، لكنه نص مُشفَّر بمعرفة صفرية لا يستطيع فكّ تشفيره؛ وينتهي (ساعة/يوم/3 أيام/7 أيام، حسب الخطة) أو يُحذف بعد أول تنزيل.",
      },
      {
        q: "هل أحتاج إلى حساب؟",
  ...
  cta: {
    text: "أرسل ملفًا مباشرةً، دون رفعه إلى سحابة أحد — دون حدّ للحجم، ودون تثبيت، ودون حاجة إلى حساب على نفس الشبكة.",
    ...
    items: [
      {
        ...
        a: "En el modo en tiempo real no: el archivo va directamente entre dispositivos y nunca se almacena en un servidor. Si creas un enlace de descarga, el servidor sí almacena algo, pero solo texto cifrado de conocimiento cero que no puede descifrar; caduca (1h/1d/3d/7d, según el plan) o se destruye tras la primera descarga.",
      },
      {
...
const pt = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: [
        "Para o caso clássico — as duas pessoas estão online agora mesmo — a transferência direta em tempo real envia até 1.000 arquivos em um único lote diretamente entre dispositivos, sem etapa de upload e sem nada armazenado em qualquer ponto intermediário. Os dois lados veem um código de verificação de 6 dígitos correspondente (SAS) para descartar um ataque de intermediário, e cada arquivo é conferido de ponta a ponta com um hash SHA-256; se a conexão cair, a transferência é retomada em vez de recomeçar.",
    ...
    items: [
      {
        ...
        a: "No modo em tempo real, não — o arquivo vai diretamente entre dispositivos e nunca é armazenado em um servidor. Se você criar um link de download, o servidor até armazena algo, mas apenas texto cifrado de conhecimento zero que ele não consegue descriptografar; ele expira (1h/1d/3d/7d, conforme o plano) ou se autodestrói após o primeiro download.",
      },
      {
