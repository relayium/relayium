...
const zh = {
  ...
  faq: {
    ...
    items: [
      ...
      {
        q: "Relayium 免费吗？",
        a: "软件是采用 AGPL-3.0 许可的免费开源软件，同一网络内的传输也不收费。托管中继与存储受每个账号都有的两项独立限制约束：每月流量额度，以及同时存放的数据量上限；超出部分由付费套餐覆盖——你也可以运行自己的节点，在任意用量下无需付费。Nextcloud 的核心服务端软件同样免费开源；Nextcloud GmbH 提供的部分托管与支持服务是收费的。",
      },
    ],
  },
  cta: {
