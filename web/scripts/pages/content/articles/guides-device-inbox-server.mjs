...
function makeDoc(x) {
  return {
    ...
    cta: { text: x.cta, button: x.button, href: "/device-inbox" },
...
const en = makeDoc({
  ...
  lead: "Device Inbox sends a file from its own page in your browser to a server, NAS or workstation you own. The browser encrypts it; only that machine decrypts and saves it.",
  ...
  cta: "Once the service reports ready, send a file from its card on Device Inbox.", button: "Open Device Inbox", related: "Continue reading",
});
...
const zh = makeDoc({
  ...
  updatedLabel: "最后更新",
  lead: "设备收件箱会把你在浏览器「设备收件箱」页面发送的文件送到自己的服务器、NAS 或工作机。浏览器负责加密，只有目标机器能够解密并写入磁盘。",
  ...
  cta: "服务显示 ready 后，就可以在「设备收件箱」页面从设备卡片发送文件。", button: "打开「设备收件箱」", related: "继续阅读",
});
...
const ja = variant(common, {
  ...
  lead: "Device Inbox は、ブラウザーの Device Inbox ページから自分のサーバー、NAS、ワークステーションへファイルを送ります。ブラウザーで暗号化され、対象のマシンだけが復号して保存できます。",
  ...
  cta: "サービスが ready になったら、Device Inbox の端末カードからファイルを送信できます。", button: "Device Inbox を開く", related: "続きを読む",
});
const ko = variant(common, {
  ...
  lead: "Device Inbox는 브라우저의 Device Inbox 페이지에서 내가 소유한 서버, NAS 또는 워크스테이션으로 파일을 보냅니다. 브라우저가 암호화하며 대상 기기만 복호화하고 저장합니다.", lead2: "서버 수신기는 SSH 창을 닫거나 재부팅해도 계속 동작해야 합니다. 권장 설치 프로그램은 폴더와 실제 systemd 서비스를 만듭니다. inbox run은 전경 진단 또는 컨테이너 진입점입니다.",
  ...
  cta: "서비스가 ready이면 Device Inbox의 기기 카드에서 파일을 보내세요.", button: "Device Inbox 열기", related: "계속 읽기",
});
const de = variant(common, {
  ...
  cta: "Sobald der Dienst ready meldet, sende eine Datei über seine Karte in Device Inbox.", button: "Device Inbox öffnen", related: "Weiterlesen",
});
const fr = variant(common, {
  ...
  lead: "Device Inbox envoie un fichier depuis sa propre page dans le navigateur vers votre serveur, NAS ou poste de travail. Le navigateur le chiffre et seule la machine cible le déchiffre et l'enregistre.",
  ...
  cta: "Quand le service indique ready, envoyez un fichier depuis sa fiche dans Device Inbox.", button: "Ouvrir Device Inbox", related: "Continuer la lecture",
});
const es = variant(common, {
  ...
  lead: "Device Inbox envía un archivo desde su propia página en el navegador a tu servidor, NAS o equipo de trabajo. El navegador lo cifra; solo la máquina de destino puede descifrarlo y guardarlo.", lead2: "El receptor debe seguir activo al cerrar SSH y después de reiniciar. El instalador recomendado crea la carpeta y un servicio systemd real. inbox run queda para diagnósticos en primer plano o contenedores.",
  ...
  cta: "Cuando el servicio indique ready, envía un archivo desde su tarjeta en Device Inbox.", button: "Abrir Device Inbox", related: "Seguir leyendo",
});
const pt = variant(common, {
  ...
  cta: "Quando o serviço mostrar ready, envie um arquivo pelo cartão no Device Inbox.", button: "Abrir Device Inbox", related: "Continue lendo",
});
const ar = variant(common, {
  ...
  lead: "يرسل Device Inbox الملف من صفحته في المتصفح إلى خادمك أو جهاز NAS أو محطة العمل. يشفّره المتصفح، ولا يفكّه ويحفظه إلا الجهاز المستهدف.", lead2: "يجب أن يستمر مستقبِل الخادم بعد إغلاق SSH وإعادة التشغيل. ينشئ المثبّت الموصى به المجلد وخدمة systemd فعلية. أما inbox run فللتشخيص في الواجهة أو للحاويات فقط.",
  ...
  cta: "عندما تعرض الخدمة ready أرسل ملفًا من بطاقته في Device Inbox.", button: "فتح Device Inbox", related: "تابع القراءة",
});
