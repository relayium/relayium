...
// released publicly under its own tag namespace (currently `macos-v1.2.2`, a
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "The macOS app is released under its own tag, macos-v1.2.2: a Developer ID-signed, Apple-notarized direct download from GitHub, not a Mac App Store listing. No version below ships it. The iOS app is an engineering build and has not been released publicly.",
...
const zh = {
  ...
  otherDocLabel: "隐私政策",
  lead: [
    "Relayium 有三种发布节奏，这一页对三种都如实说明：网页版持续部署，命令行工具带版本号打标签发布，原生应用则各自单独发布——macOS 1.2.2 已可从 GitHub 下载，iOS 应用仍是开发版，尚未公开发布。",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "macOS 应用用自己的标签 macos-v1.2.2 单独发布：这是一份经过 Developer ID 签名、通过 Apple 公证、直接从 GitHub 下载的安装包，不是 Mac App Store 上架版本。下面任何一个版本都不包含它。iOS 应用仍是开发版，尚未公开发布。",
...
const ja = {
  ...
  lead: [
    "Relayium のリリースには3つのリズムがあり、このページはその3つすべてを正直に書いています。ウェブアプリは継続的にデプロイされ、コマンドラインツールは番号を付けてリリースされ、ネイティブアプリはそれぞれ独自にリリースされます。macOS 1.2.2 は署名済みのダウンロードとして GitHub で公開されており、iOS アプリはまだ公開されていない開発ビルドです。",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "macOS アプリは macos-v1.2.2 という独自のタグで別にリリースされます。Developer ID で署名し Apple の公証を通した、GitHub からの直接ダウンロードで、Mac App Store 版ではありません。以下のどのバージョンにも含まれていません。iOS アプリは開発ビルドであり、まだ公開されていません。",
...
const ko = {
  ...
  lead: [
    "Relayium은 세 가지 리듬으로 배포되며, 이 페이지는 셋 모두를 있는 그대로 밝힙니다. 웹 앱은 계속 배포되고, 명령줄 도구는 번호를 붙여 릴리스하며, 네이티브 앱은 각자 따로 릴리스합니다. macOS 1.2.2은 서명된 다운로드로 GitHub에 공개되어 있고, iOS 앱은 아직 공개되지 않은 개발 빌드입니다.",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "macOS 앱은 macos-v1.2.2이라는 자체 태그로 따로 릴리스합니다. Developer ID로 서명하고 Apple 공증을 마친 GitHub 직접 다운로드이며, Mac App Store 등록이 아닙니다. 아래 어떤 버전에도 포함되어 있지 않습니다. iOS 앱은 개발 빌드이며 아직 공개 릴리스가 아닙니다.",
...
const de = {
  ...
  lead: [
    "Relayium erscheint in drei Rhythmen, und diese Seite benennt alle drei ehrlich: Die Web-App wird laufend ausgeliefert, die Kommandozeilenwerkzeuge bekommen Nummern und Tags, und die nativen Apps werden eigenständig veröffentlicht — macOS 1.2.2 gibt es als signierten Download bei GitHub, während die iOS-App weiterhin ein Entwicklungs-Build ist, der noch nicht öffentlich veröffentlicht wurde.",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "Die macOS-App wird unter einem eigenen Tag veröffentlicht, macos-v1.2.2: ein mit Developer ID signierter, von Apple notarisierter Direkt-Download bei GitHub, kein Mac-App-Store-Eintrag. Keine der Versionen unten enthält sie. Die iOS-App ist ein Entwicklungs-Build und wurde nicht öffentlich veröffentlicht.",
...
const fr = {
  ...
  lead: [
    "Relayium avance à trois rythmes, et cette page les décrit honnêtement tous les trois : l'application web est déployée en continu, les outils en ligne de commande sont numérotés et publiés, et les applications natives sont publiées séparément — macOS 1.2.2 se télécharge depuis GitHub, signée et notarisée, tandis que l'application iOS reste une version d'ingénierie qui n'est pas diffusée publiquement.",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "L'application macOS est publiée sous son propre tag, macos-v1.2.2 : un téléchargement direct depuis GitHub, signé Developer ID et notarisé par Apple, et non une fiche sur le Mac App Store. Aucune version ci-dessous ne la contient. L'application iOS reste une version d'ingénierie, non diffusée publiquement.",
...
const ar = {
  ...
  lead: [
    "يصدر Relayium بثلاثة إيقاعات، وهذه الصفحة تذكرها الثلاثة بصراحة: تطبيق الويب يُنشر باستمرار، وأدوات سطر الأوامر تحمل أرقامًا وعلامات إصدار، والتطبيقات الأصلية تصدر على حدة — إصدار macOS 1.2.2 متاح للتنزيل موقّعًا من GitHub، أما تطبيق iOS فما يزال نسخة تطوير لم تُنشر للعموم بعد.",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "تطبيق macOS يصدر بعلامته الخاصة macos-v1.2.2: تنزيل مباشر من GitHub موقّع بـ Developer ID وموثّق من Apple، وليس إدراجًا في Mac App Store. ولا يتضمنه أي إصدار في الأسفل. أما تطبيق iOS فنسخة تطوير لم تُنشر للعموم.",
...
const es = {
  ...
  lead: [
    "Relayium avanza a tres ritmos, y esta página los cuenta los tres con honestidad: la aplicación web se despliega de forma continua, las herramientas de línea de comandos llevan número y etiqueta, y las aplicaciones nativas se publican por su cuenta: macOS 1.2.2 ya se descarga firmada desde GitHub, mientras que la aplicación de iOS sigue siendo una compilación de ingeniería que aún no se ha publicado.",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "La aplicación de macOS se publica con su propia etiqueta, macos-v1.2.2: una descarga directa desde GitHub, firmada con Developer ID y notarizada por Apple, no una ficha en la Mac App Store. Ninguna versión de abajo la incluye. La aplicación de iOS es una compilación de ingeniería y no se ha publicado.",
...
const pt = {
  ...
  lead: [
    "O Relayium sai em três ritmos, e esta página conta os três com honestidade: o app web é implantado continuamente, as ferramentas de linha de comando recebem número e tag, e os apps nativos são publicados por conta própria — o macOS 1.2.2 já pode ser baixado assinado no GitHub, enquanto o app de iOS ainda é um build de engenharia que não foi publicado.",
  ...
  sections: [
    {
      ...
      bullets: [
        ...
        "O app de macOS é publicado com a própria tag, macos-v1.2.2: um download direto do GitHub, assinado com Developer ID e notarizado pela Apple, e não uma listagem na Mac App Store. Nenhuma versão abaixo o inclui. O app de iOS é um build de engenharia e não foi publicado.",
