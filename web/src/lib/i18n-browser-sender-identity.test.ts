...
//    (MaxBrowserDevicesPerAccount, server/account/store.go). /me lists those rows
//    under their own heading and removes them (MePage.browser-senders.test.ts),
//    so the copy must name the number central enforces, send the person to that
//    heading by its exact title, keep the other real way forward (a browser that
//    already sent from this account), no longer say it needs support, and never
//    suggest signing in again, which cannot free a place.
...
describe("browser sender identity refusals", () => {
  ...
  for (const [code, m] of Object.entries({ en, zh }) as ["en" | "zh", typeof en][]) {
    ...
    it(`${code}: the browser limit points to the /me section that frees a place`, () => {
      ...
      expect(limit, `${code}: must name the /me heading exactly`).toContain(m.me.browserSendersTitle);
      expect(limit).toContain(c.previousBrowser);
      expect(limit, `${code}: still says there is no self-service fix`).not.toMatch(c.support);
