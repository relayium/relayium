...
/** 界面提供的固定五挡（秒）。顺序即渲染顺序，从短到长。
 *
 * 最长一挡必须覆盖到最高档位的留存上限（defaultPlans 里 max = 14 天），否则
 * 付费到顶的用户在界面上永远拿不到自己已经买下的保留时间。 */
export const TTL_CHOICES = [3600, 86400, 259200, 604800, 1209600] as const;
