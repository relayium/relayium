...
const en: Messages = {
  ...
  cliPage: {
    ...
    inboxStep3Body: "enable is the opt-in, and it is made at that machine: no sender, share link or task can turn it on. run is the receiver itself, in the foreground. status tells you what is actually true — the folder, the credential, whether a worker is running, and what the server thinks.",
    ...
    inboxServiceNote: "To keep it running, relayium inbox service systemd-user | systemd-system | launchd | container prints a service definition for this exact machine and the install commands to go with it. It never installs anything itself — the system-wide form needs root, and it prints the sudo commands for you to read first. relayium inbox run is a plain foreground process: it forks nothing, writes no pid file, logs to stdout and exits cleanly on SIGTERM, which is what systemd, launchd and a container entrypoint each want.",
