...
const en: Messages = {
  ...
  account: {
    ...
    errTooLong: "That password is too long. Use at most 72 characters — fewer if it contains non-Latin letters or emoji.",
    errRegisterPendingDeletion: "An account with this email is scheduled for deletion. Log in to reactivate it, or use the link in the deletion email.",
    errSessionExpired: "You've been signed out in this browser. Sign in again, then change your password.",
  ...
  billing: {
    ...
    changeError: "Couldn't change your plan. Please try again.",
		paymentPending: "Payment needs action and may expire, so the change has not taken effect. Complete it with Stripe or update your payment method; Relayium updates automatically after payment.",
    compositePaymentPending: "Payment needs action and may expire, so neither change has taken effect and the later cycle switch was not scheduled. Complete it with Stripe or update your payment method. After payment applies, return here to schedule the later cycle switch.",
    changeSuccess: "Plan updated — thanks!",
    downgradeScheduled: "Downgrade scheduled — you'll switch at the end of this billing period.",
    cycleChangeScheduled: "Billing-cycle change scheduled — you'll switch at the end of this billing period.",
    compositeScheduled: "Your plan changed now, and the new billing cycle is scheduled for the end of this period.",
    compositePartial: "Your plan changed now, but the later billing-cycle switch was not scheduled. Please try the change again.",
    ...
    scheduledNote: (plan, cycle) => `You're scheduled to switch to ${plan}${cycle ? ` (${cycle})` : ""} at the end of this billing period.`,
  ...
  pricingPage: {
    ...
    selfhostTitle: "Carry your own traffic: run your own node",
  ...
  me: {
    ...
    browserSendersTitle: "Browsers registered for sending",
    browserSendersIntro: "Each browser that sends to your devices from this account is registered as a sender, up to 20. Removing one here doesn't sign that browser out: its next send is refused, and sending again registers it as a new sender.",
    browserSendersCount: (used, max) => `${used} of ${max} places used`,
    browserSendersEmpty: "No browser has sent from this account yet.",
    browserSenderDefaultName: "Web browser",
    browserSenderRegistered: (when) => `Registered ${when}`,
    browserSenderNotUsed: "No send recorded yet",
    browserSenderRemove: "Remove",
    browserSenderRemoveLabel: (name, ref, registered) => `Remove ${name} (${ref}, ${registered})`,
    browserSenderConfirmRemove: (name, ref, registered) =>
      `Remove the sending registration “${name}”? (${ref} · ${registered}) That browser stays signed in. Its next send is refused and nothing is sent; sending again registers it as a new sender. A send already under way in that browser may stop. Files and messages it already sent are not affected.`,
  ...
  deviceInbox: {
    ...
    sendMessageButtonLabel: (name) => `Send a message… to ${name}`,
    composerHeading: (name) => `Message to ${name}`,
    messagePlaceholder: "Type the message to send…",
    messageCount: (bytes, max) => `${bytes} of ${max} bytes`,
    messageEmptyHint: "Type something first — an empty message can't be sent.",
    messageTooLongHint: (over) => `${over} bytes over the limit. Shorten it before sending.`,
    messageSend: "Send message",
    messageSendLabel: (name) => `Send message to ${name}`,
    composerClose: "Close",
    ...
    sendErrSenderIdentity: "This browser is no longer registered to send from your account, so nothing was sent. Send again and Relayium registers this browser as a new sender automatically.",
    sendErrBrowserDeviceLimit: "Nothing was sent: this account already has the maximum of 20 browsers registered for sending. Each new browser, or a browser whose site data was cleared, takes one of these places. Remove browsers you no longer use under “Browsers registered for sending” on your account page, then send again — or send from a browser that has sent from this account before. Signing in again won't change this.",
    ...
    sendErrTextUnsupported: "That device doesn't show messages, so nothing was queued. Files can still be sent to it.",
  ...
  why: {
    ...
    selfhostTitle: "Run your own node and carry your own traffic",
    selfhostBody: "You can run your own relay / storage node and bind it to your account. What it carries — relayed bytes, and uploads stored on it — stops using your monthly traffic allowance, though stored files still count toward your storage cap. Downloads Relayium carries for you still count as usual, including downloads of files stored on your node.",
  ...
  accountDelete: {
    ...
    lead: "You opened the link to confirm deleting your Relayium account. Nothing has happened yet — it starts only when you press the button below.",
    consequence: "Confirming signs this account out everywhere — browsers, apps and the CLI — removes its devices, and erases the files and data it stored on Relayium's servers, right away. The account itself is permanently deleted when the grace period ends; the email Relayium sends next gives the date. Until then you can undo this by signing in again, or with the reactivation link in that email.",
    cta: "Delete my account",
    working: "Deleting…",
    keep: "Keep my account",
    done: "Your account is scheduled for deletion, and you have been signed out.",
    doneUndo: "Changed your mind? Until the grace period ends, sign in again or use the reactivation link in the email Relayium sends to keep the account.",
    invalid: "This link has expired or was already used. Deletion links work once, for one hour. To delete your account, sign in and request a new link from your personal center.",
    errNetwork: "Couldn't reach Relayium. Check your connection and try again.",
    errServer: "Something went wrong on Relayium's side and the deletion wasn't confirmed. Try again in a moment.",
    noToken: "This page is only reachable from an account-deletion email.",
    home: "Back to home",
  },
  accountReactivate: {
    title: "Reactivate your account",
    lead: "Your account is scheduled for deletion. Reactivate it to cancel the deletion and sign in on this device.",
    cta: "Reactivate my account",
    working: "Reactivating…",
    done: "Your account is active again — signing you in…",
    invalid: "This reactivation link has expired or was already used. If your account is still scheduled for deletion, sign in and Relayium will offer to reactivate it.",
    errServer: "Something went wrong. Try signing in — if your account is still scheduled for deletion, Relayium will offer to reactivate it.",
    noToken: "This page is only reachable from a reactivation email.",
    home: "Back to home",
  },
  nav: { footerLegalLabel: "Legal", footerGuidesLabel: "Resources", primaryLabel: "Main", footerLabel: "Footer", lanTab: "LAN", crossTab: "Cross-network transfer", offlineTab: "Share a link", cliTab: "CLI", appsTab: "Apps", deviceInboxTab: "Device Inbox", toolsLabel: "Downloads and tools" },
  shell: {
    groupDirect: "Live transfer",
    groupLinks: "Links",
    groupThisDevice: "Your devices",
    more: "More",
    lanShort: "LAN",
    crossShort: "Cross-network",
    offlineShort: "Share",
    ...
    nameRow: "Visible name",
    statusRow: "Status",
    statusReady: "Ready",
    statusConnecting: "Connecting…",
    statusReconnecting: "Reconnecting…",
    selectDevice: (n) => `Select ${n}`,
    scanning: "Looking for devices on this network…",
    notScanning: "Not looking for devices — this page is not connected to Relayium right now. Nearby devices appear once the connection is ready.",
  },
  crossTitle: "Cross-network transfer",
  crossSubtitle: "Both devices online, joined by a 6-digit pairing code — on the same network or across networks.",
  offlineTitle: "Share a link",
  ...
  cliPage: {
    ...
    sections: {
      ...
      modes: "Modes at a glance",
      guides: "Guides",
      reference: "Command reference",
      security: "Security & integrity",
      faq: "FAQ",
    },
    installIntro: "One binary, no runtime. Put it on every machine you want to move files between.",
    installPosixLabel: "macOS and Linux",
    installWindowsLabel: "Windows",
    installWindowsBody:
      "Windows gets a portable ZIP, not the command above — that installer is a POSIX shell script and does not run on Windows. Unzip it and put relayium.exe anywhere on your PATH; nothing is registered and there is nothing to uninstall.",
    ...
    tasksIntro:
      "Two questions decide the mode: can the machine on the other end be offline right now, and is it a machine you administer?",
    tasks: {
      offline: {
        title: "Another device can be offline",
        body: "Relayium holds the encrypted file until the other end comes for it. Both of these need an account, and both store ciphertext only — Relayium holds no key that could open it.",
      },
      bothOnline: {
    ...
    modes: {
      ...
      inbox: {
        ...
        notes: [
          ...
          "There is no official Relayium container image. A published image is a supply-chain artifact that needs its own signing and provenance, and an unsigned one would be worse than none — so the container path is inbox run as the entrypoint of an image you build.",
        ],
      },
      text: {
      ...
      serve: {
        ...
        notes: [
          "Trust is this host's authorized_fingerprints file and nothing else. Being logged in to a Relayium account grants no one filesystem access here, and logging in does not authorize a pusher.",
          "In a terminal, serve asks you to approve each new pusher on its first push and remembers it. With no terminal — a systemd unit, a pipe — an unknown pusher is rejected instead; pre-authorize it with relayium authorize <fingerprint>, using the same --config-dir the listener runs with. A running listener picks the new fingerprint up on its next connection, with no restart.",
          "The listener's own fingerprint is pinned by the pusher on first contact, and a later change is refused rather than trusted.",
          "--bind is empty by default, which listens on every interface including public ones and relies on your firewall; pass --bind 127.0.0.1, or a private address, to limit the listener itself. --dir must already exist and be writable — serve never creates it.",
        ],
      },
      sync: {
    ...
    compareIntro:
      "The four things the modes genuinely differ on. The table scrolls sideways; it is also a keyboard stop, so the right-hand columns are reachable without a mouse.",
    compareCaption: "Relayium CLI modes compared by account, availability, path and verification",
    compareModeHeader: "Mode",
    compareColumns: {
      account: "Account",
      online: "Other end online?",
      path: "Where the bytes go",
      verify: "Verification",
    },
    compare: {
      cloud: {
        account: "up: yes · down: no",
        online: "No — stored until it is fetched",
        path: "Ciphertext stored by Relayium",
        verify: "The key stays in the link's #k= fragment; the server never receives it",
      },
      inbox: {
        account: "Both ends, same account",
        online: "No — queued until the device returns",
        path: "Ciphertext queued by Relayium, sealed to that device",
        verify: "The device checks the file against its manifest before it writes",
      },
      text: {
      ...
      serve: {
        ...
        verify: "authorized_fingerprints; the listener is pinned on first contact",
      },
      sync: {
        account: "None",
    ...
    guideGroups: {
      ...
      bothOnline: "When both devices are online",
      managed: "Machines you manage",
    },
    guides: {
      ...
      cloudAsync: "Push files to the cloud, pull them on another computer",
      receiveFromCli: "Receive files from the command line",
      deviceInboxServer: "Set up Device Inbox on an always-on Linux server",
      automateBackups: "Schedule an off-host server copy with a cron job",
    },
    referenceIntro:
      "Flags, and exactly which commands take each one. This table scrolls sideways and is a keyboard stop.",
    thFlag: "Flag",
    thApplies: "Commands",
    thMeaning: "Meaning",
    flags: {
      dir: "Directory to receive into. serve defaults to the current directory and never creates it — it must already exist and be writable. inbox enable requires it and creates it 0700.",
      bind: "Address serve listens on. Empty, the default, listens on every interface including public ones and relies on your firewall.",
    ...
    trustH3: "Files on disk",
    trustIntro:
      "Identity and trust live in ~/.config/relayium/, or wherever --config-dir points — /etc/relayium for a system service, for instance:",
    trustFiles: {
      identity: "this host's persistent direct-transfer identity, created on first use and kept 0600. relayium id prints its fingerprint.",
      knownHosts: "fingerprints of listeners you have pushed to, pinned on first contact and refused if they later change.",
      authorized: "on a listener, the only pushers allowed to connect — the whole trust decision for serve.",
    },
    faq: {
      account: {
    ...
    inbox: {
      stepsLabel: "Set up the receiving machine",
      steps: [
        {
          label: "Install or update the CLI on the receiving machine.",
          body: "relayium update --check reports whether a newer release exists; relayium update installs it in place. Device Inbox needs a build that has the inbox command — relayium inbox --help is the quickest way to be sure.",
        },
        {
          label: "Sign in on that machine.",
          body: "This is what makes it a device in your account. The label you pass is what My Devices will show, so give the machine a name you will recognise later; without the flag it registers this host's own name. The terminal prints the exact label before you approve it in the browser.",
        },
        {
          label: "Choose a folder and start receiving.",
  ...
  crossnet: {
    ...
    relayQuotaWarn: "You've used up this month's traffic allowance, so no relay is offered until it resets next month — not even through your own nodes — and uploads to hosted storage pause too (an upload that lands on your own node still works, but downloading it through Relayium needs allowance). Cross-network browser sessions can still connect when a direct path between the two devices works. Same-network sessions and direct CLI transfers are unaffected, though this account can't create new CLI send or text codes until then. To be sure a cross-network send goes through now, upgrade for more traffic.",
    relayQuotaFail: "This cross-network browser session couldn't connect: no relay is available because this month's traffic allowance is used up, and no direct path between the two devices worked. Same-network sessions and direct CLI transfers are unaffected. Upgrade for more traffic, or wait for next month's reset.",
  ...
  crossSell: {
    realtime: { lead: "Is the other person online right now? Use cross-network transfer — live and end-to-end encrypted, on the same network or across networks; its TURN relay carries only ciphertext it cannot decrypt.", cta: "Go to cross-network transfer →" },
    offline: { lead: "Recipient not online? Share a link — encrypt, upload, and leave a download link they can fetch for days.", cta: "Go to Share a link →" },
  },
  methods: {
    ...
    stored: { name: "Share a link", sub: "Your browser encrypts then stores; the recipient downloads anytime, no live session and no account needed.", badge: "Offline OK" },
  },
  pair: {
  ...
  storedRecv: {
    ...
    reconnecting: (attempt, max) => `Connection lost — reconnecting and continuing from where it stopped (attempt ${attempt} of ${max})…`,
    done: "Saved.",
    dismiss: "Dismiss",
    retry: "Try again",
  ...
  download: {
    ...
    resuming: (attempt, max) => `Connection lost — reconnecting and continuing from where it stopped (attempt ${attempt} of ${max})…`,
  ...
  howItWorks: {
    ...
    offline: {
      title: "Share a link, in three steps",
  ...
  compare: {
    ...
    sub: "LAN needs no account but only works on one shared network; cross-network transfer hands small files and text over in real time while both sides are online, whether or not they share a network; sharing a link is the one to use for large files, or when they'll fetch it later.",
    colFeature: "Aspect",
    colLan: "LAN",
    colRealtime: "Cross-network",
    colStored: "Share a link",
    rows: [
  ...
  faq: {
    ...
    offline: [
      ...
      { q: "Can I store files on my own server?", a: "Yes. Deploy your own node from the account page and your uploads go to your own node first, without using your monthly traffic allowance or daily upload quota — the files still count toward your storage cap. They fall back to public nodes if yours is offline, full or unreachable, or fail instead if you turn on “Only use my own nodes for relay/storage”. Downloads of those files are carried by Relayium and count as usual." },
    ],
  },
