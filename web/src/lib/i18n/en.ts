...
const en: Messages = {
  ...
  crossnet: {
    ...
    relayQuotaWarn: "You've used up this month's traffic allowance, so no relay is offered until it resets next month — not even through your own nodes — and new stored links are paused too. Cross-network browser sessions can still connect when a direct path between the two devices works. Same-network sessions and direct CLI transfers are unaffected, though this account can't create new CLI send or text codes until then. To send across networks now, upgrade for more traffic.",
