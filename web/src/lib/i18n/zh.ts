...
const zh: Messages = {
  ...
  cliPage: {
    ...
    mode2Tag: "免费 · 直连 P2P",
    mode2Body:
      "跨网络发送给另一个人。先用 relayium login 登录一次，然后直接 send、不带码：CLI 会生成一个 6 位配对码（字母表里没有 0 和 1），5 分钟内有效，并打印出对面要执行的完整命令。把这个码线下转告对方——电话里念一下即可。这个码不能自己指定，服务器只认它自己签发的；接收方不需要账号。两端都必须是 CLI；如果对方只有浏览器，请改用 relayium up。连接是点对点直连：只有一次很小的会合握手经过 Relayium 用来撮合两端——文件字节绝不经过。如果双方都在严格 NAT 之后、无法直连，传输就会直接失败（CLI 没有中继）。两端终端都会打印一个由固定 TLS 证书指纹推导出的 6 位 SAS 码。通过带外渠道核对一致，可以确认指纹未被替换、会合服务没有冒充任一端点；它认证的是端点，而不是网络路径上的每一跳（加上 --verify 可要求在传输任何字节前先确认）。",
