...
const zh: Messages = {
  ...
  tagline: "端到端加密的点对点传输，文件与文本皆可 · 服务器两者都看不到",
  ...
  maxSize: (s) => `最大 ${s}`,
  footer: "端到端加密（X25519 + AES-256-GCM）· 信令服务器只转发连接信息，看不到文件内容，也看不到消息",
  ...
  crossnet: {
    realtimeTitle: "实时直传",
    realtimeSub: "对方此刻在线 · 端到端加密实时传输 · 服务器看不到你的文件",
    realtimeFoot: "接收方无需账号 · 端到端加密",
    signInToSend: "登录后即可跨网络发送。接收方无需账号。",
    relayQuotaWarn: "本月中继流量已用尽。点对点直连仍可用——仅中继兜底不可用。下月初重置；现在要发送，可改用下载链接、升级套餐以获得更多中继流量，或运行你自己的节点。",
    relayQuotaFail: "直连失败，且本月中继流量已用尽——无中继可兜底。下月初重置；可改用下载链接、升级套餐以获得更多中继流量，或运行你自己的节点。",
  },
  offline: {
  ...
  crossSell: {
    realtime: { lead: "对方就在线？实时直传更快——端到端加密、服务器看不到你的文件。", cta: "前往实时直传 →" },
    offline: { lead: "对方现在不在线？用异步传输——加密上传后生成下载链接，对方几天内随时来取。", cta: "前往异步传输 →" },
  },
  methods: {
    realtime: { name: "⚡ 实时直传", sub: "选好文件生成 6 位码，念码、发链接或扫码任选其一；对方加入即自动开始传输。", badge: "接收方免注册" },
  ...
  howItWorks: {
    realtime: {
      ...
      ways: [
        ...
        { icon: "⚡", name: "加入即自动开传", how: "对方加入后传输自动开始，全程端到端加密；跨网络时数据流经加密 TURN 中继转发，中继只经手无法解密的密文。", tag: "服务器看不到你的文件" },
      ],
    },
    offline: {
      title: "异步传输，三步完成",
  ...
  compare: {
    ...
    rows: [
      ...
      { label: "文件是否经服务器", realtime: "同一网络：否，设备间直连；跨网络：经加密中继，中继只见密文", stored: "是，但仅存零知识密文" },
  ...
  faq: {
    ...
    home: [
      { q: "两台设备必须在同一网络吗？", a: "首页的局域网直传要求双方在同一 Wi-Fi / 局域网内。如果不在同一网络，请用「跨网络直传」的配对码/分享链接，或用异步下载链接。" },
    ...
    cross: [
      { q: "配对码和分享链接有什么区别？", a: "两者都是端到端加密的实时直传。配对码适合双方同时在线、口头或即时交换一小段数字；分享链接则可以直接发给对方点开。选顺手的即可。" },
      { q: "双方必须同时在线吗？", a: "是。跨网络实时直传需要收发双方同时在线并保持页面打开。若无法同时在线，请改用异步下载链接——它把加密文件先存下，对方随后再取。" },
      { q: "配对码会过期吗？", a: "会。配对码是一次性、短时有效的，过期或用过后需要重新生成，以降低被他人猜到的风险。" },
      { q: "跨网络要经过你们的中继，会泄露文件吗？", a: "不会。跨网络传输经加密 TURN 中继转发，中继只转发密文、无法解密，看不到任何文件内容。" },
      { q: "为什么发送方要登录，接收方不用？", a: "跨网络直连要用到我们的信令与中继资源（有成本），所以发送方需要登录以便计量与防滥用；接收方只是点开链接或输入配对码，永远不需要账号。" },
      { q: "速度由什么决定？想更快怎么办？", a: "直连时取决于双方网络的上下行带宽和延迟；走中继时还受中继节点带宽影响。在「个人中心」部署你自己的节点，可以显著提升跨网络的中继速度，而且免费。" },
    ],
    offline: [
