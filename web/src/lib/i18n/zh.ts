...
const zh: Messages = {
  ...
  descDefault:
    "Relayium 是开源、端到端加密的文件传输工具。在浏览器里实时直传：同一网络下两台设备直连、文件不经过服务器，跨网络则经加密中继转发（中继只见密文）。无需安装，同一网络下无需账号，跨网络配对时仅发送方需登录。同一条连接也能传文本——一段备注、一个链接、一条命令、一整段代码——双方都在线即可，消息从不存到服务器。",
  ...
  pricingPage: {
    navLink: "定价",
    title: "简单、诚实的定价",
    subtitle: "同一网络内的传输始终免费。跨网络中继与云端存储按每月额度计量——每个账号都有额度，付费套餐额度更高。你也可以通过运行自己的节点，保持 100% 免费。",
    signedOutCta: "登录即可订阅或更改套餐。",
    freeTitle: "始终免费",
    freeLead: "零成本传输文件，永远免费：",
    free1: "局域网（LAN）传输——即时完成，无需账号。",
    free2: "使用命令行工具的点对点直传——走你自己的 SSH、daemon 直连或配对码——不计量。只有 send 生成配对码时需要账号；接收永远不需要。",
    free3: "端到端加密，无文件大小限制，文件在设备之间直接传输——绝不经过我们的服务器。",
    freeWhy: "直传和局域网传输对我们没有任何成本，因此对所有人始终免费——接收文件无需注册。",
    paidTitle: "你付费的是什么",
    paidLead: "付费套餐只覆盖两项真正产生成本的服务：",
    paid1: "中继带宽——浏览器里的跨网络传输经加密中继转发，因此能穿透严格的防火墙与 NAT。每个账号都有每月免费额度，付费套餐额度更高。这会消耗真实的带宽。",
    ...
    faqTitle: "常见问题",
    q1: "真的免费吗？",
    a1: "是的。同一网络内的传输与命令行的点对点直传均为端到端加密、无限量且完全免费——接收文件甚至无需账号。跨网络中继与云端存储则计入每月额度，包含免费档在内的每个账号都有额度；只有需要更多时才付费。",
    q2: "直传和中继传输有什么区别？",
    a2: "直传会把文件从一台设备直接发送到另一台——最快，而且免费；同一网络下的浏览器传输和命令行走的就是这条路。浏览器里的跨网络传输则经加密中继转发，以便可靠穿透严格的防火墙与 NAT——中继只经手密文，而它消耗的带宽正是每月额度与付费套餐所覆盖的部分。",
    q3: "为什么会有付费档位？",
  ...
  me: {
    ...
    nodesTrafficHint: "「中继」只统计 P2P 直连打不通、走 TURN 中继的流量；用离线/异步上传存到节点的文件计入「存储」，不会增加中继量。上传时系统会优先使用你自己的在线节点。",
    checkNode: "测试连通",
  ...
  features: {
    ...
    items: [
      ...
      { title: "服务器看不到你的文件", desc: "实时模式下数据经 WebRTC 传输：同一网络下在设备间直接流动、不经过服务器，跨网络则经加密中继转发（中继只见密文）；可选的下载链接仅为零知识加密暂存。" },
  ...
  compare: {
    ...
    rows: [
      ...
      { label: "有效期", realtime: "即传即走，不留存", stored: "最长 14 天（按套餐），或阅后即焚" },
  ...
  useCases: {
    ...
    items: [
      ...
      { icon: "💬", title: "只想递一段文本，而不是文件", desc: "Wi-Fi 密码、注册链接、一条命令、一段报错堆栈、一整块代码——输入或粘贴即可端到端加密送到另一台设备。双方在线即可，不留存。" },
    ],
  },
  faq: {
    title: "常见问题",
    ...
    items: [
      ...
      { q: "服务器能看到我的文件吗？", a: "不能。同一网络下实时直传的文件根本不经过服务器；跨网络时经加密中继转发，中继也只转发密文、无法解密。下载链接在浏览器端先加密，服务器只保存无法解密的密文，密钥只存在于链接的分享者与接收者之间。" },
      ...
      { q: "能只发一段文本，不发文件吗？", a: "可以——文本在这里是与文件并列的传输方式，而不是聊天。在设备卡片上点「发送消息」，输入或粘贴备注、链接、命令或多行代码，它会沿同一条点对点连接端到端加密送达。它是实时的，所以双方设备必须同时在线；它只作用于这两台设备的这一次会话；并且任何内容都不会写入服务器——Relayium 不保存消息记录。每条消息最多 65,536 个 UTF-8 字节，更大的内容请当作文件发送。" },
    ],
    home: [
  ...
  homeCross: {
    title: "不在同一个网络？",
    desc: "对方在线，用实时直传（服务器看不到你的文件）；对方不在线，用异步传输（加密暂存，凭链接随时取）。",
    realtimeCta: "实时直传 →",
    offlineCta: "异步传输 →",
  },
  homeText: {
    title: "要递过去的，未必是文件",
    sub: "文本走的是同一条加密连接：一段备注、一个网址、一条命令、一段报错日志、一整块代码。选一台设备，点「发送消息」，输入或粘贴即可——不必先存成文件，中间也不必再借第三方应用。",
    points: [
      "与文件传输完全一样：端到端加密、点对点，并且只作用于这两台设备的这一次会话。",
      "实时进行：双方设备必须同时在线，且各自都开着页面。",
      "从不存到任何服务器，会话结束即消失——Relayium 不是聊天工具，不保存消息记录。",
    ],
  ...
  text: {
    ...
    ephemeralNote: "消息端到端加密，不存储在任何服务器上，会话结束即消失。",
