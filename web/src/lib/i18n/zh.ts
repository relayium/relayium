...
const zh: Messages = {
  ...
  pricingPage: {
    ...
    free3: "端到端加密：文件没有大小限制，实时文本绝不存储。直连时，文件与消息在设备之间直接传输——绝不经过我们的服务器。",
    ...
    faqTitle: "常见问题",
    q1: "真的免费吗？",
    a1: "是的。同一网络内的文件或实时文本传输与命令行直传均为端到端加密且完全免费——接收文件或加入文本会话甚至无需账号。文件不限大小；临时文本只在双方在线时传输且绝不存储。浏览器跨网络中继与云端存储则计入每月额度，包含免费档在内的每个账号都有额度；只有需要更多时才付费。",
    q2: "直传和中继传输有什么区别？",
