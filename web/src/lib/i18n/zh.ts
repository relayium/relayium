...
const zh: Messages = {
  ...
  descCross: "用 6 位配对码跨网络传输文件——实时传输、端到端加密；中转服务器只能看到密文，看不到文件内容。",
  ...
  account: {
    ...
    pendingDeletion: "你的账户已排入删除队列。点击恢复以保留它。",
    reactivate: "恢复账户",
  ...
  pricingPage: {
    navLink: "定价",
    title: "简单、诚实的定价",
    subtitle: "同一网络内的传输始终免费。跨网络中继与云端存储按每月额度计量——每个账户都有额度，付费套餐额度更高。你也可以通过运行自己的节点，保持 100% 免费。",
    signedOutCta: "登录即可订阅或更改套餐。",
    freeTitle: "始终免费",
    freeLead: "零成本传输文件，永远免费：",
    free1: "局域网（LAN）传输——即时完成，无需账户。",
    free2: "使用命令行工具的点对点直传——走你自己的 SSH、守护进程直连或配对码——不计量。只有 send 生成配对码时需要账户；接收永远不需要。",
    free3: "端到端加密，无文件大小限制，文件在设备之间直接传输——绝不经过我们的服务器。",
    freeWhy: "直传和局域网传输对我们没有任何成本，因此对所有人始终免费——接收文件无需注册。",
    paidTitle: "你付费的是什么",
    paidLead: "付费套餐只覆盖两项真正产生成本的服务：",
    paid1: "中继带宽——浏览器里的跨网络传输经加密中继转发，因此能穿透严格的防火墙与 NAT。每个账户都有每月免费额度，付费套餐额度更高。这会消耗真实的带宽。",
    ...
    selfhostCta: "在你的账户中添加节点 →",
    faqTitle: "常见问题",
    q1: "真的免费吗？",
    a1: "是的。同一网络内的传输与命令行的点对点直传均为端到端加密、无限量且完全免费——接收文件甚至无需账户。跨网络中继与云端存储则计入每月额度，包含免费档在内的每个账户都有额度；只有需要更多时才付费。",
    q2: "直传和中继传输有什么区别？",
    ...
    q4: "我能完全不付费吗？",
    a4: "可以——运行你自己的节点。由于 Relayium 是开源的，你可以在自己掌控的硬件上托管中继和存储，并让每一次传输都经过它，没有任何限制和费用。在账户页面即可添加。",
    q5: "以后可以更改套餐吗？",
  ...
  me: {
    ...
    nodeOnline: "在线",
    nodeOffline: "离线",
    nodeRelayed: (bytes) => `已中转 ${bytes}`,
    nodeStored: (bytes) => `已存储 ${bytes}`,
    nodeFreeTag: "免费",
    nodeStorageFree: (free, total) => `磁盘可用 ${free} / 共 ${total}`,
    nodesTrafficHint: "「中转」只统计 P2P 直连打不通、走 TURN 中继的流量；用离线/异步上传存到节点的文件计入「存储」，不会增加中转量。上传时系统会优先使用你自己的在线节点。",
    checkNode: "测试连通",
  ...
  why: {
    heading: "关于账号",
    costTitle: "为什么要登录？",
    costBody: "这个板块对我们有实实在在的成本（中转带宽 / 存储）。登录后我们会给你一份免费额度；额度用完后继续使用需要付费。",
    selfhostTitle: "想完全免费？自建节点",
    selfhostBody: "你可以自己搭一个中转 / 存储节点，绑定到你的账号。之后你的传输全部走你自己的节点、不经过我们——我们不经手，也就不收费。",
  ...
  faq: {
    ...
    cross: [
      ...
      { q: "速度由什么决定？想更快怎么办？", a: "直连时取决于双方网络的上下行带宽和延迟；走中继时还受中继节点带宽影响。在「个人中心」部署你自己的节点，可以显著提升跨网络的中转速度，而且免费。" },
    ],
    offline: [
