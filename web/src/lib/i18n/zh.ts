...
const zh: Messages = {
  ...
  billing: {
    ...
    popular: "最受欢迎",
    save2mo: "免费赠送 2 个月",
    changeConfirm: (name) => `切换到${name}？差价将在你的下一张账单中按比例结算。`,
    downgradeConfirm: (name) => `降级到${name}？将在当前计费周期结束时生效——在此之前你仍保留当前套餐，且不予退款。`,
  ...
  pricingPage: {
    ...
    faqTitle: "常见问题",
    q1: "真的免费吗？",
    a1: "是的。局域网和点对点直传均为端到端加密、无限量且完全免费——接收文件甚至无需账户。只有中继传输和云端存储才需要付费套餐。",
    q2: "直传和中继传输有什么区别？",
    a2: "直传会把文件从一台设备直接发送到另一台——最快，而且免费。当无法建立直接连接时（严格的防火墙或 NAT），Relayium 会回退到通过服务器中继加密数据流，这会消耗带宽，也正是付费套餐所覆盖的部分。",
    q3: "为什么会有付费档位？",
  ...
  crossnet: {
    realtimeTitle: "实时直传",
    realtimeSub: "对方此刻在线 · 点对点直连 · 服务器看不到你的文件",
  ...
  crossSell: {
    realtime: { lead: "对方就在线？实时直传更快——点对点直连、服务器看不到你的文件。", cta: "前往实时直传 →" },
  ...
  howItWorks: {
    realtime: {
      ...
      ways: [
        ...
        { icon: "⚡", name: "加入即自动开传", how: "对方加入后传输自动开始，点对点直连、端到端加密；打洞失败时经加密 TURN 中继转发，依然无法被解密。", tag: "服务器看不到你的文件" },
      ],
    },
    offline: {
      title: "异步传输，三步完成",
  ...
  faq: {
    ...
    cross: [
      ...
      { q: "直连失败走中继，会泄露文件吗？", a: "不会。直连打不通时会自动退回加密 TURN 中继，中继只转发密文、无法解密，看不到任何文件内容。" },
