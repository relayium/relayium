...
const zh: Messages = {
  ...
  step4: "对方接受后，核对两边校验码一致，即可开始文件传输或文本会话。",
  ...
  cliPage: {
    ...
    mode2Tag: "免费 · 直连 P2P",
    mode2Body:
      "跨网络发送给另一个人。先用 relayium login 登录一次，然后直接 send、不带码：CLI 会生成一个 6 位配对码（字母表里没有 0 和 1），30 分钟内有效，并打印出对面要执行的完整命令。把这个码线下转告对方——电话里念一下即可。这个码不能自己指定，服务器只认它自己签发的；接收方不需要账号。两端都必须是 CLI；如果对方只有浏览器，请改用 relayium up。连接是点对点直连：只有一次很小的会合握手经过 Relayium 用来撮合两端——文件字节绝不经过。如果双方都在严格 NAT 之后、无法直连，传输就会直接失败（CLI 没有中继）。两端终端都会打印一个由固定 TLS 证书指纹推导出的 6 位 SAS 码。通过带外渠道核对一致，可以确认指纹未被替换、会合服务没有冒充任一端点；它认证的是端点，而不是网络路径上的每一跳（加上 --verify 可要求在传输任何字节前先确认）。",
  ...
  methods: {
    realtime: { name: "实时传输", sub: "选好文件生成 6 位码，念码、发链接或扫码任选其一；对方加入即自动开始传输。", badge: "接收方免注册" },
  ...
  features: {
    ...
    items: [
      ...
      { title: "防中间人", desc: "两边屏幕显示同一段浏览器 SAS；核对一致可检测 X25519 端点公钥是否被替换，或服务器是否冒充了任一端点。" },
  ...
  howItWorks: {
    realtime: {
      ...
      ways: [
        { name: "创建配对码", how: "登录并创建一个 6 字符配对码，同时得到加入链接和二维码；等另一台设备加入后再选择要发送的内容。", tag: "创建者需登录" },
        { name: "把码给对方", how: "念码、发链接或让对方扫码，任选一种；对方在任何现代浏览器里输入或点开即可加入。", tag: "码 30 分钟有效" },
        { name: "选择文件或文本", how: "对方加入后，在设备卡上选择最多 1,000 个文件，或点「发送消息」发送在线文本。核对 SAS 后，内容以端到端加密密文经 TURN 发送。", tag: "中继无法读取或解密两者" },
      ],
    },
    offline: {
      title: "异步传输，三步完成",
  ...
  useCases: {
    ...
    items: [
      ...
      { title: "隐私敏感的一次性投递", desc: "端到端加密加上 SAS 校验码防中间人，配合阅后即焚，适合传合同、证件、密钥等敏感文件。" },
  ...
  workspace: {
    ...
    stateOpen: "已连接",
    stateFailed: "连接失败",
    lanesNote: "文件和消息共用这一条已验证的连接。校验码只需核对一次；每一批文件、每一次会话仍会单独征求同意。",
