...
const zh: Messages = {
  ...
  howItWorks: {
    realtime: {
      ...
      ways: [
        { icon: "🔗", name: "创建配对码", how: "登录并创建一个 6 字符配对码，同时得到加入链接和二维码；等另一台设备加入后再选择要发送的内容。", tag: "创建者需登录" },
        { icon: "🔢", name: "把码给对方", how: "念码、发链接或让对方扫码，任选一种；对方在任何现代浏览器里输入或点开即可加入。", tag: "码 5 分钟有效" },
        { icon: "⚡", name: "选择文件或文本", how: "对方加入后，在设备卡上选择最多 1,000 个文件，或点「发送消息」发送在线文本。核对 SAS 后，内容以端到端加密密文经 TURN 发送。", tag: "中继无法读取或解密两者" },
      ],
    },
    offline: {
      title: "异步传输，三步完成",
      ...
      ways: [
        ...
        { icon: "🔗", name: "生成下载链接", how: "可设 1 小时到最长 14 天有效期（上限取决于套餐），或阅后即焚；解密密钥藏在链接的 # 片段里，不会发送到服务器。", tag: "有效期可控" },
  ...
  useCases: {
    ...
    items: [
      ...
      { icon: "💬", title: "只想递一段文本，而不是文件", desc: "Wi-Fi 密码、注册链接、命令、报错或代码——输入或粘贴即可端到端加密送达。双方需在线；Relayium 不保留服务端记录，但接收端可留存。" },
    ],
  },
  faq: {
    title: "常见问题",
