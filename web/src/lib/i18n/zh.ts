...
const zh: Messages = {
  ...
  features: {
    ...
    items: [
      ...
      { title: "服务器看不到你的文件或消息", desc: "实时数据经 WebRTC 传输：同一网络下直连，跨网络中继也只见密文。文本从不存储；可选下载链接只保存零知识加密的文件密文。" },
  ...
  faq: {
    ...
    items: [
      ...
      { q: "能只发一段文本，不发文件吗？", a: "可以——文本在这里是与文件并列的传输方式，而不是聊天。在设备卡片上点「发送消息」，输入或粘贴备注、链接、命令或多行代码，它会通过独立的点对点连接端到端加密送达。它是实时的，所以双方设备必须同时在线；它只作用于这两台设备的这一次会话；并且任何内容都不会写入服务器——Relayium 不保存消息记录。每条消息最多 65,536 个 UTF-8 字节，更大的内容请当作文件发送。" },
    ],
    home: [
