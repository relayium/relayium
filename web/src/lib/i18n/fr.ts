...
const fr: Messages = {
  ...
  cliPage: {
    ...
    inboxStep3Body: "enable est le consentement explicite, et il se donne sur cette machine : aucun expéditeur, lien de partage ou tâche ne peut l'activer. run est le récepteur lui-même, au premier plan. status dit ce qui est réellement vrai — le dossier, les identifiants, si un worker tourne, et ce que le serveur en pense.",
    ...
    inboxServiceNote: "Pour qu'elle tourne en permanence, relayium inbox service systemd-user | systemd-system | launchd | container imprime une définition de service pour cette machine précise, avec les commandes d'installation correspondantes. Elle n'installe jamais rien elle-même : la forme système exige root, et les commandes sudo sont affichées pour que vous les lisiez d'abord. relayium inbox run est un simple processus au premier plan : pas de fork, pas de fichier pid, journal sur stdout, arrêt propre sur SIGTERM — exactement ce qu'attendent systemd, launchd et un point d'entrée de conteneur.",
  ...
  download: {
    ...
    cli: {
      ...
      windowsNote: "Le bloc POSIX ci-dessus ne couvre pas Windows. Ce bloc PowerShell choisit l'archive officielle v0.16.0 adaptée au processeur, vérifie son SHA-256 épinglé avant extraction, l'exécute depuis un répertoire temporaire, puis supprime ce répertoire :",
