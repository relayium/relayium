...
const de: Messages = {
  ...
  cliPage: {
    ...
    inboxStep3Body: "enable ist die ausdrückliche Zustimmung, und sie wird an dieser Maschine gegeben: weder ein Absender noch ein Freigabelink noch eine Aufgabe kann sie einschalten. run ist der Empfänger selbst, im Vordergrund. status sagt, was tatsächlich zutrifft – Ordner, Zugangsdaten, ob ein Worker läuft und was der Server annimmt.",
    ...
    inboxServiceNote: "Damit es dauerhaft läuft, gibt relayium inbox service systemd-user | systemd-system | launchd | container eine Service-Definition genau für diese Maschine samt der passenden Installationsbefehle aus. Installiert wird nichts davon automatisch – die systemweite Variante braucht root, und die sudo-Befehle werden zum Mitlesen ausgegeben. relayium inbox run ist ein gewöhnlicher Vordergrundprozess: kein fork, keine PID-Datei, Log auf stdout, sauberes Ende bei SIGTERM – genau das, was systemd, launchd und ein Container-Entrypoint erwarten.",
