...
const ja: Messages = {
  ...
  cliPage: {
    ...
    mode1Tag: "free",
    mode1Body:
      "すでに ssh でログインできるマシン——VPS、自宅サーバー、ワークステーションなど——へ（または、から）ファイルをコピーします。バイトは SSH 接続上を流れ、Relayium のサーバーには一切触れません。アカウントも不要です。リモートに relayium がインストールされていればネイティブプロトコル（ファイル単位の再開＋SHA-256）を使用し、なければ単純な tar ストリームにフォールバックします。",
    ...
    step1Label: "リスナー側:",
    step1Body: "serve を起動し、relayium id でこのホストのフィンガープリントを表示する。",
    step2Label: "初回プッシュ時に承認する:",
    step2Body:
      "ターミナル上で、リスナー側が新しいプッシュ側のアドレスとフィンガープリントを表示し、承認するか尋ねます——y と答えれば記憶され、以降のプッシュは確認なしで通ります。サービスとして動かしていてターミナルがない場合は、relayium authorize で事前に承認してください。",
    step3Label: "プッシュ側:",
    step3Body:
      "relayium://host へ push する。リスナーの鍵は初回接続時に信頼され known_hosts にピン留めされます。以降フィンガープリントが変わると、黙って受け入れずに拒否されます。",
    refH2: "リファレンス",
    flagsH3: "共通フラグ",
    thFlag: "フラグ",
    thApplies: "対象",
