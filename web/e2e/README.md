...
- `mixed-link.mjs`（opt-in）：统一工作区头部、390px 下的 40 文件同意卡、文本通道同意后
  两条通道都活着的状态。
- `code-room.mjs`（CI）：配对码房间里的统一工作区（390px）、它的**阿拉伯语 RTL** 分支、
  以及文件同意卡。RTL 那一格在这里而不在别处：配对码房间是手机最常走的一条路。
...
1. `483920` —— 统一工作区本身：一个动作、一条链路、一个 SAS、两条通道、RTL、断开。
...
收方仍然独立同意、落盘字节按 SHA-256 逐一核对；阿拉伯语 RTL 下 390px 不横向溢出；
