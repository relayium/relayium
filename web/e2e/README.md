...
- `mixed-link.mjs` —— 两个新版本浏览器在 LAN 房间里的统一链路（`link/1`），
  ...
  现在是**三幕**，各自拥有自己的标签页和自己那份冻结 act 台账：二十幕的统一链路旅程、
  多页设备身份（一个浏览器的两页 + 第三台设备），以及有界的中继池失败（两页，外加一份
  只答 `/api/ice` 的池形状应答）。为什么分幕而不是加 act，见下面 C3b-7 / C3b-8 两节。
...
审计当初认定搁浅的有**八条**。八条全部改了状态，所以现在是**零条搁浅、五条已托管
...
| 8 | 有界的中继池失败（池里发了凭据然后被丢掉） | **本地已迁移，待托管 CI**（C3b-8） |

"零条搁浅"说的是这张迁移清单，不是 `lan-transfer.mjs`——那个文件还在，也还是跑不起来。
删它是下面的阶段四，要等三幕全部在托管 `main` 上绿了之后。
...
- *顺手加的反空转机制*：一幕不等于一条断言。`mixed-link.mjs` 当时加了一份**冻结的逐 act
  ...
  **第三幕**，都不是往 `ACTS` 里再塞 act，所以现在是三份冻结清单、三个字面量计数——
  `ACTS`/`EXPECTED_ACT_COUNT`（20）、`MULTIPAGE_ACTS`/`EXPECTED_MULTIPAGE_ACT_COUNT`（5）
  与 `RELAY_ACTS`/`EXPECTED_RELAY_ACT_COUNT`（4），`EXPECTED_SCENARIO_COUNT` 为 3。
  三份刻意不合并：一份二十九条的平表会把"某一幕根本没起来"和"少了一个 act"报成同一种
  失败，而这两者要修的地方不一样。
...
**当前状态：本地已独立验过——含真浏览器旅程、Web 全量、`npm run check` 与生产构建——
但托管 CI 还没跑过它。** 所以第 8 条仍记作"本地已迁移待托管 CI"：要等 `mixed-link-e2e`
在 `main` 上跑过第三幕；本节任何一句都不是 exact-main 或托管结论。在最终源码上跑了什么：
...
**阶段三：有界的中继失败——C3b-8 已本地迁移，待托管 CI。** 第 7 条在 C3b-7 搬走后，这一
阶段只剩第 8 条。它放到最后，因为它需要前面用不到的搭台：那份池形状的 `/api/ice` 响应、
一个连不上的 TURN 主机，以及一个真的会走完的探测预算。

**阶段四：删掉 `lan-transfer.mjs` 和它的 `test:e2e` npm 脚本。** 只在托管 `main` 带着
阶段一到三全绿之后——按 C3b-8 之后的状态，也就是只在第三幕在 `main` 上跑过
`mixed-link-e2e` 之后。删早了会丢掉那条还在等这次托管跑的断言；第 1、2、3、6、7、8 条
搬走且第 4、5 条按上面记录的确定性证据退役后，已经没有搁浅的了。而留着比没用更糟——一个
永远退不出 0 的脚本，教会所有人忽略一次红。
...
很难回来——但它确实是个缺口，而且是这条尾巴里**唯一**还没被迁走的东西。这里以前写的是
"浏览器侧的'不支持对端'形状和独有断言第 8 条一起在等阶段三"；第 8 条已经在 C3b-8 搬走，
所以剩下的就是那个形状，它是一件单独的活，不属于上面任何一行编号。这里不需要真的做一次
老式传输，因为已经没有老式传输可做。
...
能单独失效），第 17-21 条是第二幕自己的**五个** act。每个 act 在自己的断言全部通过之后
调一次 `act(name, message)`——它同时负责打印那行 ✓ 和记台账，所以"报告成功"和"记录为
执行过"是同一句话，拆不开。两幕共用同一个 `newLedger(acts)` 工厂，所以"名字不在清单里"
和"同一个 act 记了两次"这两道守卫不可能在一幕里有、在另一幕里被忘掉。

跑完 `runScenarios()` 对**每一幕**三样一起查：成员、**顺序**，以及那一幕自己的字面量
计数——`EXPECTED_ACT_COUNT = 20` 和 `EXPECTED_MULTIPAGE_ACT_COUNT = 5`。这里必须是
字面量，不能写 `ACTS.length`——数组和它自己的长度在有人删掉一项之后仍然彼此同意，于是
一次少了一幕的运行会干干净净地报 `19/19`。同样的道理，`EXPECTED_SCENARIO_COUNT = 2`
单独存在，但它保护不了多少：一个被改到只剩第一条断言的 `mixedScenario` 照样能让整套
报 `2/2`。真正起作用的仍然是那两个 act 数。两份清单刻意不合并成一份二十五条的平表，
因为那样"多页那一幕根本没起来"和"少了一个 act"会报成同一种失败。

`e2e/go-server.test.mjs` 对着这个 runner 的**源码**钉住整个形状（每次推送都跑）：
两份冻结的 act 名单与顺序、三个字面量计数（以及"清单项里不许出现 `.length`"）、
...
不许再长回答控件循环、以及不许用信令帧读数或退役选择器抄近路。
