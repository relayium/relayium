...
删它是下面的阶段四，现在只等一件事：C3b-9 迁过来的那条"渲染出来的不支持对端"形状，
本地已绿、还没在托管 `main` 上跑过。
...
**C3b-9 —— 连不上的那个对端，究竟被告知了什么。** 这是那条只属于老路的断言里**渲染出来
的那一半**（见下面"两件这次迁移不许做的事"），也是托管 `main` 和阶段四之间最后剩下的
东西。它是这个 runner 的**第四幕**
...
但托管 CI 还没跑过它。** 所以那条"渲染出来的不支持对端"形状仍记作**本地**迁移待托管 CI，
本节任何一句都不是 exact-main 或托管结论。下面每一道门都是审阅方在被审阅的源码上跑的，
它取代了本节原来那份作者侧垫片记录：
...
这条线，C3b-9 还没有。删早了会丢掉那条还在等这次托管跑的断言；第 1、2、3、6、7、8 条
...
- **C3b-9 已迁移，本地绿，尚未托管**：`capsSuppressedScenario` 当初是设计来在真浏览器
  里钉观察方那一侧的形状（从不通告 caps 的对端拿不到任何控件，老对端那边也不冒出任何
...
也就是说：源码那一半一直有门守着，渲染出来那一半现在本地也覆盖上了。在 `mixed-link-e2e`
把第四幕在托管 `main` 上跑过之前，本文件必须继续把渲染那一半记作**本地**迁移：缺口是变
窄了，不是关上了。这里以前写的是"浏览器侧的'不支持对端'形状和独有断言第 8 条一起在等
阶段三"；第 8 条在 C3b-8 搬走、这个形状在 C3b-9 搬走，它仍然是一件单独的活，不属于上面
