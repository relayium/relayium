...
审计当初认定搁浅的有**八条**。其中七条已经改了状态，所以现在是**一条搁浅、四条已托管
...
| 7 | 多页设备身份与焦点（一个浏览器的两页 + 第三台独立设备） | **本地已迁移，待托管 CI**（C3b-7） |
...
- *C3b-1 当时没搬什么*：第 1、2、3、5 条都没被那一刀改动，但这四条后来都改了状态——
  ...
  第 7 条随后在 C3b-7 搬走，所以**只剩第 8 条**，它是阶段三剩下的部分。逐字节续传和
...
- *顺手加的反空转机制*：一幕不等于一条断言。`mixed-link.mjs` 当时加了一份**冻结的逐 act
  ...
  那次活扫描确实落在"接受"和"强制掐断"之间。C3b-7 加的是**第二幕**，不是第二十一个 act，
  所以现在是两份冻结清单、两个字面量计数——`ACTS`/`EXPECTED_ACT_COUNT`（20）与
  `MULTIPAGE_ACTS`/`EXPECTED_MULTIPAGE_ACT_COUNT`（5），`EXPECTED_SCENARIO_COUNT` 为 2。
  两份刻意不合并：一份二十五条的平表会把"多页那一幕根本没起来"和"少了一个 act"报成同一种
  失败，而这两者要修的地方不一样。
...
**当前状态：本地已端到端验过，含真浏览器旅程；托管 CI 还没跑过它。** 所以第 7 条仍记作
"本地已迁移待托管 CI"，下面任何一句都不是 exact-main 或托管结论。
...
每处变异都被还原成变异前的原样，所以这一刀的改动仍然只有上面点名的那七个文件。这一整轮
**不能**代表的是托管道次：`mixed-link-e2e` 还没在 `main` 上跑过这一幕，所以第 7 条还不是
已托管覆盖。

**阶段三：有界的中继失败。** 第 7 条已在 C3b-7 搬走，所以这一阶段只剩第 8 条。它放到最后，
因为它需要前面用不到的搭台：那份池形状的 `/api/ice` 响应、一个连不上的 TURN 主机，以及一个
真的会走完的探测预算。

**阶段四：删掉 `lan-transfer.mjs` 和它的 `test:e2e` npm 脚本。** 只在托管 `main` 带着
阶段一到三全绿之后。删早了会丢掉仍然搁浅在里面的那几条——第 1、2、3、6、7 条搬走且第 4、5 条按上面
记录的确定性证据退役后还剩一条；而留着比没用更糟——一个永远退不出 0 的
脚本，教会所有人忽略一次红。
...
很难回来——但它确实是个缺口，浏览器侧的"不支持对端"形状和独有断言第 8 条一起在等阶段三。
