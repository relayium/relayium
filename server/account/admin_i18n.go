...
var adminEN = map[string]string{
	...
	"分配":           "Assign",
	// Time-bounded administrator membership grants. The two warning strings are
	// the load-bearing ones: an authority is a BINDING to a payment channel, not
	// a live subscription, and the English must not quietly upgrade it into one.
	"限时会员授权":      "Timed membership grant",
	"⚠ 存在支付渠道授权：": "⚠ Payment-channel authority on record: ",
	"（仅代表账号已绑定该渠道，不代表当前有生效订阅）": " (means only that the account is bound to that channel — NOT that a subscription is currently active)",
	"到期":           "expires",
	"已过期":          "expired",
	"授权套餐":         "Grant plan",
	"天数（1-1000）":   "Days (1-1000)",
	"授权方式":         "Grant mode",
	"从现在起（替换现有授权）": "From now (replaces any existing grant)",
	"在现有未过期授权上顺延":  "Extend the current unexpired grant",
	"授权":           "Grant",
	"⚠ 该账号已存在支付渠道状态，本次授权不会改动它": "⚠ This account already has payment-provider state, and this grant does not change it",
