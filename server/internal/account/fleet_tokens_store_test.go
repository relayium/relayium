...
func TestFleetTokenLifecycle(t *testing.T) {
	...
	tok := FleetToken{ID: "ft1", TokenHash: authx.HashToken("raw-secret"), Name: "sh-1", CreatedAt: 100}
	if err := st.CreateFleetToken(ctx, tok); err != nil {
		t.Fatalf("create: %v", err)
	}

	// Resolvable by hash while active.
	got, ok, err := st.FleetTokenByHash(ctx, authx.HashToken("raw-secret"))
	...
	if _, ok, _ := st.FleetTokenByHash(ctx, authx.HashToken("raw-secret")); ok {
