...
func TestNodeViewsOnlineFlag(t *testing.T) {
	...
	views := nodeViews(nodes, nil, now, Settings{})
	if len(views) != 2 {
...
func TestNodeViewsStorage(t *testing.T) {
	now := time.Unix(10000, 0)
	views := nodeViews([]Node{{ID: "n", LastSeenAt: now.Unix(), StorageEnabled: true, StorageTotal: 20 << 30, StorageFree: 5 << 30, StoredBytes: 3 << 30}}, nil, now, Settings{})
