...
const rolloutCols = `track, target_version, current_node_id, byo_batch, stage_started_at, status, halted_reason`
...
func (s *SQLiteStore) GetRolloutTrack(ctx context.Context, track string) (RolloutTrack, bool, error) {
	...
	err := s.reader().QueryRowContext(ctx,
		...
		Scan(&t.Track, &t.TargetVersion, &t.CurrentNodeID, &t.ByoBatch, &t.StageStartedAt, &t.Status, &t.HaltedReason)
...
func (s *SQLiteStore) PutRolloutTrack(ctx context.Context, t RolloutTrack) error {
	_, err := s.db.ExecContext(ctx,
		...
		t.Track, t.TargetVersion, t.CurrentNodeID, t.ByoBatch, t.StageStartedAt, t.Status, t.HaltedReason)
	return err
}
