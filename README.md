...
**1.4.1**: a Developer ID-signed, Apple-notarized and stapled build, published
...
[SSH retirement and reopening notes](docs/CLI-SSH-RETIREMENT.md).
...
Keep a folder mirrored with **`sync`** — over daemon-direct, incremental (only changed files transfer), optionally `--delete` to mirror and `--watch` to re-sync in real time:

```sh
relayium sync ./site relayium://host --delete --watch
```

For `--delete`, the receiving listener must have been started with
`--allow-delete`; otherwise nothing is deleted and the sender is told so.
Deletion is confined to the
...
**resumed**, at version `0.4.1`. It has never been publicly released — there is
